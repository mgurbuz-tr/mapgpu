...
import { classifyVisibleLayers, renderVectorLayer, renderVectorTileLayer, renderCustomShaderLayer, renderClusterLayer, renderDynamicPointLayer, renderOverlayLayer, animateView } from '../mode-helpers.js';
import type { AnimationHandle, OverlayCacheEntry } from '../mode-helpers.js';
import { isVideoOverlayLayer } from '../../interfaces/index.js';
import { lonLatToMercator } from '../coordinates.js';

const HALF_WORLD = 20037508.342789244;
const MIN_CAMERA_CLEARANCE_METERS = 5;
...
export class Mode3D implements IViewMode {
  ...
  private _frameTileCache = new Map<number, { z: number; x: number; y: number }[]>();
  /** Cached flat tile coverage keyed by floored zoom level. */
  private _frameFlatCache = new Map<number, { z: number; x: number; y: number }[]>();
  ...
  private _getCachedTiles(
    ...
    if (globeness >= 0.5) {
      ...
      if (!cached) {
        cached = this._tileCovering.getTilesForGlobe(this._transform, z);
  ...
  private _syncCameraSurfaceConstraint(activeTerrainLayer: ITerrainLayer | null = this._activeTerrainLayer): boolean {
    ...
    const baseClearanceMeters = activeTerrainLayer ? MIN_CAMERA_CLEARANCE_METERS : 0;
