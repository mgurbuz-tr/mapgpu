...
import type {
  CameraState, ICustomShaderLayer, ITerrainLayer,
...
export class Mode3D implements IViewMode {
  ...
  setState(state: Partial<ViewState>): void {
    ...
    if (state.zoom !== undefined) {
      ...
      this._projection.updateFromZoom(state.zoom);
    }
  ...
  renderFrame(ctx: RenderFrameContext): void {
    ...
    if (tileSources.length > 0) {
      const visibleTiles = globeness >= 0.5
        ? this._tileCovering.getTilesForGlobe(this._transform, zoom)
        : this._getTilesForFlat(zoom);

      if (activeTerrainLayer) {
        terrainManager.requestTiles(activeTerrainLayer, visibleTiles);
      }

      // Atmosphere + pole caps (behind tiles)
      if (globeness > 0.01) {
        renderEngine.drawAtmosphere(globeness);
        renderEngine.drawPoleCaps([0.65, 0.78, 0.88, globeness]);
      }
      ...
      for (const tile of readyTiles) {
        ...
        const mercNorth = 1.0 - (tile.extent[3] + HALF) / (2 * HALF);
        const mercSouth = 1.0 - (tile.extent[1] + HALF) / (2 * HALF);
        ...
        renderEngine.drawGlobeTile({
          ...
          heightExaggeration: useTerrain
            ...
            : undefined,
          lighting3D: useTerrain
            ? activeTerrainLayer.lighting3D
            : undefined,
        });
      }
    } else if (globeness > 0.01) {
      renderEngine.drawAtmosphere(globeness);
      renderEngine.drawPoleCaps([0.65, 0.78, 0.88, globeness]);
    }
    ...
    for (const layerId of vectorTileLayerIds) {
      ...
      const vtCoords = globeness >= 0.5
        ? this._tileCovering.getTilesForGlobe(this._transform, tileZoom)
        : this._getTilesForFlat(tileZoom);
    ...
    for (const layerId of vectorLayerIds) {
      renderVectorLayer(layerId, renderEngine, layerManager, bufferCache, true, this._transform.zoom);
    }
  ...
  private _computeClusterExtent3857(): [number, number, number, number] | null {
    ...
    if (validCount < 3 || !isFinite(minX) || !isFinite(minY) || !isFinite(maxX) || !isFinite(maxY)) {
  ...
  private _getTilesForFlat(targetZoom: number): { z: number; x: number; y: number }[] {
    ...
    const pitchFactor = 1 / Math.max(0.3, Math.cos(this._transform.pitch * Math.PI / 180));
    const halfX = Math.ceil(this._transform.viewportWidth / 256 / 2) + 1;
    const halfY = Math.ceil(this._transform.viewportHeight / 256 / 2 * pitchFactor) + 1;
