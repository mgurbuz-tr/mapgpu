...
describe('Mode3D', () => {
  ...
  describe('getState / setState', () => {
    ...
    it('keeps zoom fixed when pitch increases without terrain', () => {
      mode.setState({ center: [35, 39], zoom: 22, pitch: 85 });

      expect(mode.getState().zoom).toBe(22);
      expect((mode as any)._transform.cameraSurfaceDistanceMeters).toBeLessThan(5);
    });
  ...
  describe('interaction', () => {
    it('does not auto-zoom out on right-drag pitch when terrain is inactive', () => {
      ...
      expect(interactionMode.getState().zoom).toBe(22);
      expect(markDirty).toHaveBeenCalled();
      expect(onViewChange).toHaveBeenCalled();
