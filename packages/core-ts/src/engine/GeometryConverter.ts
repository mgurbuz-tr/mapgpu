...
export class GeometryConverter {
  ...
  static modelInstancesFromFeatures(
    ...
    for (const feature of features) {
      ...
      const scale = (attrs.scale as number) ?? defaultScale;
  ...
  private static _extractLines(geometry: GeometryLike, out: number[][]): void {
    const coords = geometry.coordinates;
  ...
  private static _triangulatePolygon(
    ...
    for (let r = 0; r < rings.length; r++) {
      ...
      const ring = rings[r]!;
      for (const c of ring) {
        const [mx, my] = coordinateToMercator(c, spatialReference);
    ...
    // Add vertices (as vec3)
    const vertexStart = outVertices.length / 3;
    for (let i = 0; i < flatCoords.length; i += 2) {
      outVertices.push(flatCoords[i]!, flatCoords[i + 1]!, 0);
    }
