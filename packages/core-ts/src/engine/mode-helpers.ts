...
export function classifyVisibleLayers(layerManager: LayerManager, zoom?: number): ClassifiedLayers {
  ...
  return { tileSources, terrainLayerIds, vectorLayerIds, customLayerIds, clusterLayerIds, dynamicPointLayerIds, vectorTileLayerIds, overlayLayerIds };
}
...
export function renderOverlayLayer(
  ...
  if (isImageOverlayLayer(layer)) {
    ...
    const cached = overlayCache.get(layerId);
    if (cached && cached.videoWidth === video.videoWidth && cached.videoHeight === video.videoHeight) {
  ...
  if (globe) {
    ...
    const mercNorth = 1.0 - (y1 + MERC_HALF) / (2 * MERC_HALF);
    const mercSouth = 1.0 - (y0 + MERC_HALF) / (2 * MERC_HALF);
