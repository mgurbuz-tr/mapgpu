...
export class VerticalPerspectiveTransform {
  ...
  private _flatViewProjectionMatrix = new Float32Array(16);
  ...
  constructor(params?: Partial<GlobeCameraParams>) {
    if (params) {
      ...
      if (params.pitch !== undefined) this._pitch = params.pitch;
      if (params.bearing !== undefined) this._bearing = params.bearing;
      if (params.fov !== undefined) this._fov = params.fov;
  ...
  computeNearFar(): [number, number] {
    const dist = this.cameraToCenterDistance;
    const near = Math.max(0.01, dist * 0.5);
  ...
  screenToLonLat(sx: number, sy: number): [number, number] | null {
    ...
    let len = Math.sqrt(rayDirEye[0] ** 2 + rayDirEye[1] ** 2 + rayDirEye[2] ** 2);
  ...
  lonLatToScreen(lon: number, lat: number): [number, number] | null {
    ...
    const px = cosLat * Math.sin(lonRad);
    const py = Math.sin(latRad);
    const pz = cosLat * Math.cos(lonRad);

    // Check clipping plane
    const cp = this._clippingPlane;
    if (px * cp[0] + py * cp[1] + pz * cp[2] + cp[3] < 0) {
      return null; // Behind globe
    }

    // Transform through view-projection matrix
    const vp = this._viewProjectionMatrix;
    const clipX = vp[0]! * px + vp[4]! * py + vp[8]! * pz + vp[12]!;
    const clipY = vp[1]! * px + vp[5]! * py + vp[9]! * pz + vp[13]!;
    ...
    const screenY = (1 - ndcY) * 0.5 * this._viewportHeight;
  ...
  lonLatToScreenFlat(lon: number, lat: number): [number, number] | null {
    ...
    // lon/lat → Mercator [0..1]
    const latRad = lat * DEG2RAD;
    const mx = (lon + 180) / 360;
    const sinLat = Math.sin(latRad);
    // Guard against ±90° latitude (tan → ∞)
    const clamped = Math.max(-0.9999, Math.min(0.9999, sinLat));
    const my = (1 - Math.log((1 + clamped) / (1 - clamped)) / (2 * Math.PI)) / 2;

    // Apply flat VP matrix: flatVP * (mx, my, 0, 1)
    const fp = this._flatViewProjectionMatrix;
    const clipX = fp[0]! * mx + fp[4]! * my + fp[12]!;
    const clipY = fp[1]! * mx + fp[5]! * my + fp[13]!;
    const clipW = fp[3]! * mx + fp[7]! * my + fp[15]!;
...
/**
 * WebGPU perspective matrix: NDC z ∈ [0, 1] (NOT OpenGL [-1, 1]).
...
function perspectiveMat4(
  ...
  out[0] = f / aspect;
  out[5] = f;
  out[10] = far * rangeInv;           // WebGPU: maps near→0, far→1
  out[11] = -1;
  out[14] = near * far * rangeInv;    // WebGPU: -(near*far)/(far-near)
}
