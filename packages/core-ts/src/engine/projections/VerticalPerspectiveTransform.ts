...
export class VerticalPerspectiveTransform {
  ...
  computeNearFar(): [number, number] {
    const dist = this.cameraToCenterDistance;
    // Tighter near plane at high zoom improves depth precision significantly.
    // At dist=0.05 (zoom ~22): near=0.01, far=2.05, ratio=205
    // At dist=1.0 (zoom ~8):   near=0.1,  far=3.0,  ratio=30
    const near = Math.max(0.001, dist * 0.1);
