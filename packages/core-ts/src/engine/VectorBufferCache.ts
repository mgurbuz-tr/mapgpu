...
export class VectorBufferCache {
  ...
  private _layerTerrainVersions = new Map<string, number>();
  ...
  getOrBuild(
    ...
    const context = zoom !== undefined ? { renderMode, zoom, resolution: 0 } : undefined;
  ...
  private _buildWallGroup(
    ...
    const outlineFeatures: RenderFeature[] = [
      ...
      } as RenderFeature,
      {
        ...
        geometry: {
          type: 'LineString',
          coordinates: [bottomCoords[bottomCoords.length - 1]!, topCoords[topCoords.length - 1]!],
        },
        attributes: {},
      } as RenderFeature,
    ];
  ...
  private _interleaveWallMesh(mesh: { positions: Float32Array; normals: Float32Array; vertexCount: number }): Float32Array {
    ...
    for (let i = 0; i < mesh.vertexCount; i++) {
      ...
      out[dst6 + 1] = 1.0 - (my + HALF_CIRCUMFERENCE) / (2 * HALF_CIRCUMFERENCE);
...
function merc01ToLonLat(m01x: number, m01y: number): [number, number] {
  ...
  const mercY = (1.0 - m01y) * 2 * HALF_CIRCUMFERENCE - HALF_CIRCUMFERENCE;
