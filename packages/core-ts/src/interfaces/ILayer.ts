...
export type ElevationMode = 'absolute' | 'relative-to-ground' | 'on-the-ground';

export interface ElevationInfo {
  mode: ElevationMode;
  /** Additional vertical offset in meters. */
  offset?: number;
  /**
   * How terrain height is sampled for the geometry.
   * - `'geometry'` (default): each vertex gets its own terrain height → geometry follows terrain surface
   * - `'centroid'`: terrain height is sampled once at the geometry centroid → all vertices get the same offset, geometry stays flat
   */
  sampling?: 'geometry' | 'centroid';
}
...
export interface ITerrainLayer extends ILayer {
  ...
  /** CPU-side terrain height sampling at a geographic coordinate. */
...
/** Image overlay — a single static image pinned to geographic bounds. */
export interface IImageOverlayLayer extends ILayer {
  readonly bounds: [number, number, number, number];
  readonly imageData: ImageBitmap | null;
}

/** Video overlay — a video element pinned to geographic bounds. */
export interface IVideoOverlayLayer extends ILayer {
  readonly bounds: [number, number, number, number];
  readonly videoElement: HTMLVideoElement | null;
}

export function isImageOverlayLayer(layer: ILayer): layer is IImageOverlayLayer {
  return layer.type === 'image-overlay' && 'imageData' in layer;
}

export function isVideoOverlayLayer(layer: ILayer): layer is IVideoOverlayLayer {
  return layer.type === 'video-overlay' && 'videoElement' in layer;
}
