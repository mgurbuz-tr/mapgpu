...
import type { ClusterStyleConfig } from '@mapgpu/core';
...
export class DrawDelegateCluster {
  ...
  // Per-layer state
  private layerStates = new Map<string, ClusterLayerState>();
  ...
  setSource(layerId: string, points: Float32Array, version: number): void {
    ...
    if (existing && existing.sourceVersion === version) return; // No change
   ...
   */
  private _createBitmapDigitAtlas(W: number, H: number, cell: number): Uint8Array {
