...
import type { CustomDrawCall } from '@mapgpu/core';
...
export class DrawDelegateCustom {
  private customPipelines: Map<string, CustomPipeline> = new Map();
  /** Keys for which pipeline creation failed — prevents retrying every frame */
  private customPipelineErrors: Set<string> = new Set();
  ...
  drawCustom(call: CustomDrawCall): void {
    ...
    if (this._customDrawDbgCount < 3) {
      console.log('[CP4-BIND]', {
        ...
        group2_customSize: call.customUniforms ? (call.customUniforms instanceof ArrayBuffer ? call.customUniforms.byteLength : (call.customUniforms as Float32Array).byteLength) : null,
