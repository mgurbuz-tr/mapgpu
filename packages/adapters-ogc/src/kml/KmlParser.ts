 ...
 */
export function parseKml(kmlString: string): KmlParseResult {
...
function _parseStyles(doc: Document): Map<string, KmlStyle> {
  ...
  for (let i = 0; i < mapEls.length; i++) {
    ...
    for (let p = 0; p < pairs.length; p++) {
      const keyEl = pairs[p]!.getElementsByTagName('key')[0];
      if (keyEl?.textContent?.trim() === 'normal') {
        const urlEl = pairs[p]!.getElementsByTagName('styleUrl')[0];
...
function _parseStyleElement(el: Element): KmlStyle {
...
function _kmlColorToRgba(kmlColor: string): [number, number, number, number] {
  ...
  const a = parseInt(kmlColor.slice(0, 2), 16);
