...
export function parseGpx(gpxString: string): GpxParseResult {
  ...
  for (let i = 0; i < wptEls.length; i++) {
    ...
    const coords: number[] = ele !== null ? [lon, lat, ele] : [lon, lat];
  ...
  for (let i = 0; i < trkEls.length; i++) {
    ...
    for (let s = 0; s < segEls.length; s++) {
      const pts = segEls[s]!.getElementsByTagName('trkpt');
      const coords: number[][] = [];
      for (let p = 0; p < pts.length; p++) {
        coords.push(_pointCoords(pts[p]!));
      }
