...
export class TooltipWidget {
  ...
  private _permanents = new Map<string, PermanentTooltip>();
  private _stickyEl: HTMLElement;
  ...
  private _repositionOne(tooltip: PermanentTooltip): void {
    ...
    if (!view || typeof view.toScreen !== 'function') return;

    const screen = view.toScreen(tooltip.coordinate[0], tooltip.coordinate[1]);
    if (!screen) {
