...
import type { IWidget, IView, WidgetPosition } from '@mapgpu/core';
...
export class DockPanel extends WidgetBase {
  ...
  private _collapsed = new Set<DockPosition>();
  private _panelEls = new Map<DockPosition, HTMLDivElement>();
  private _toggleBtns = new Map<DockPosition, HTMLButtonElement>();
  ...
  addWidget(widget: IWidget, dockPosition: DockPosition): void {
    ...
    if (panelEl && !this._collapsed.has(dockPosition)) {
      ...
      wrapper.setAttribute('data-dock-widget-id', widget.id);
  ...
  private _rebuildPanelContent(dockPosition: DockPosition): void {
    ...
    for (const entry of widgets) {
      const wrapper = document.createElement('div');
      wrapper.setAttribute('data-dock-widget-id', entry.widget.id);
