...
import { SimpleRenderer, IncrementalWallBuffer } from '@mapgpu/core';
import type { IRenderEngine, Mesh3DRenderBuffer, Mesh3DSymbol, ElevationInfo } from '@mapgpu/core';
import type { IWallElevationSampler } from '@mapgpu/core';
...
export class WallLayer extends GraphicsLayer {
  ...
  getWallSymbol(): Mesh3DSymbol {
    ...
    const color = (sym && 'color' in sym ? sym.color : [255, 109, 58, 60]) as [number, number, number, number];
    return {
      type: 'mesh-3d',
      meshType: 'box',
      color,
