 ...
 */
function readFeature(pbf: Pbf, end: number): MvtRawFeature {
 ...
 */
function decodeGeometry(geometry: number[]): number[][][] {
  const rings: number[][][] = [];
...
export function projectedGeometryToPublicGeometry(geometry: ProjectedGeometry): Geometry {
  const toPublicCoord = (coord: number[]): number[] => {
    ...
    return coord[2] !== undefined ? [lon, lat, coord[2]] : [lon, lat];
  };
