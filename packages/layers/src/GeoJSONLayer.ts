...
export interface GeoJSONLayerOptions extends LayerBaseOptions {
  ...
  /** Callback invoked for each loaded feature. Use to bind popups, tooltips, or extra attributes. */
  onEachFeature?: (feature: Feature) => void;
}
...
export class GeoJSONLayer extends LayerBase implements IQueryableLayer, IFeatureLayer {
  ...
  private readonly _onEachFeature?: (feature: Feature) => void;
  ...
  protected async onLoad(): Promise<void> {
    ...
    if (this._onEachFeature) {
      for (const f of this.features) this._onEachFeature(f);
    }
   ...
   */
  addData(data: GeoJsonFeatureCollection): void {
    const newFeatures = this.parseFeatureCollection(data);
    if (this._onEachFeature) {
      for (const f of newFeatures) this._onEachFeature(f);
    }
