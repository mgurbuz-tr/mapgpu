 ...
 *
 */
export class MultipointUtils {
    private static readonly _className: string = "MultipointUtils";
    public static readonly Hatch_ForwardDiagonal: number = 2;
    public static readonly Hatch_BackwardDiagonal: number = 3;
    public static readonly Hatch_Vertical: number = 4;
    public static readonly Hatch_Horizontal: number = 5;
    public static readonly Hatch_Cross: number = 8;
     ...
     * @param shapes
     */
    static addHatchFills(tg: TacticalGraphic, shapes: Array<ShapeInfo>): void {
        try {
            ...
            let hatchStyle: number = tg.fillStyle;
            let j: number = 0;
            let hatch2: number = 0;
            let shape2: Shape2;
            let index: number = 0;
            let hatchLineThickness: number = (tg.lineThickness * 0.75) * tg.patternScale;//1.5 + ((tg.lineThickness / 2) - 1.5) * tg.patternScale;
            let hatchColor: Color = tg.lineColor;
            let hatchSpacing: number = Math.trunc(hatchLineThickness * 6);//Math.trunc(hatchLineThickness * 10);

            //            if(armyc2.c5isr.JavaTacticalRenderer.MultipointUtils.isClosedPolygon(lineType)==false)
            //                if(armyc2.c5isr.JavaTacticalRenderer.MultipointUtils.IsChange1Area(lineType, null)==false)
            //                    return;
            ...
            switch (lineType) {
                ...
                case TacticalLines.LAA: {
                    hatchStyle = MultipointUtils.Hatch_BackwardDiagonal;
                    break;
                }
                ...
                case TacticalLines.RAD: {
                    ...
                    hatchLineThickness = (tg.lineThickness * 0.85) * tg.patternScale;
                    //hatchLineThickness = 1.5 + (tg.lineThickness - 1.5) * tg.patternScale;
            ...
            for (let k: number = 0; k < n; k++) {
                ...
                if (hatchStyle < MultipointUtils.Hatch_ForwardDiagonal) {
                ...
                if (tg.useHatchFill) {
                    let hatchImg = PatternFillRenderer.MakeHatchPatternFill(hatchStyle, hatchSpacing, hatchLineThickness as number, hatchColor);
                    shape.setPatternFillImage(hatchImg);
                }
                else {
                    if (hatchStyle !== MultipointUtils.Hatch_Cross) {
                        let shape3: Shape2 = MultipointUtils.buildHatchArea(tg, shape, hatchStyle, hatchSpacing);
                    ...
                    {
                        let shapeBk: Shape2 = MultipointUtils.buildHatchArea(tg, shape, MultipointUtils.Hatch_BackwardDiagonal, hatchSpacing);
                        let shapeFwd: Shape2 = MultipointUtils.buildHatchArea(tg, shape, MultipointUtils.Hatch_ForwardDiagonal, hatchSpacing);
            ...
            if (exc instanceof Error) {
                ErrorLogger.LogException(MultipointUtils._className, "addHatchFills",
                    exc);
            } else {
                throw exc;
            }
     ...
     * @return
     */
    static buildHatchArea(tg: TacticalGraphic, shape: ShapeInfo, hatchStyle: number, spacing: number): Shape2 {
        ...
        try {
            ...
            let x0: number = rect.getX();
            let y0: number = rect.getY();
            let width: number = rect.getWidth();
            let height: number = rect.getHeight();
            //we need a square
            if (width > height) {
            ...
            let horizLimit: number = 0;
            let vertLimit: number = 0;
            let j: number = 0;
            let vertPts: Array<POINT2> = new Array();
            let horizPts: Array<POINT2> = new Array();
            let vertPt: POINT2;
            let horizPt: POINT2;
            if (hatchStyle === MultipointUtils.Hatch_BackwardDiagonal) {
            ...
            if (hatchStyle === MultipointUtils.Hatch_Vertical) {
            ...
            if (hatchStyle === MultipointUtils.Hatch_Horizontal) {
                horizLimit = Math.trunc(width / (spacing / 2));
                vertLimit = Math.trunc(height / (spacing / 2));
                for (j = 0; j < vertLimit; j++) {
                    if (j % 2 === 0) {
            ...
            if (exc instanceof Error) {
                ErrorLogger.LogException(MultipointUtils._className, "buildHatchArea",
                    exc);
            } else {
                throw exc;
            }
    ...
    protected static POINT2ToPoint(pt2: POINT2): Point {
        ...
        pt.x = pt2.x as number;
        pt.y = pt2.y as number;
        return pt;
    }
     ...
     * @return 
     */
    protected static addModifiersBeforeClipping(tg: TacticalGraphic): boolean {
     ...
     * @deprecated
     */
    protected static FilterPoints(tg: TacticalGraphic): void {
        try {
            let lineType: number = tg.lineType;
            ...
            for (j = 1; j < n; j++) {
                ...
                dist = LineUtility.calcDistance(lastGoodPt, currentPt);
                switch (lineType) {
                    case TacticalLines.LC: {
            ...
            if (exc instanceof Error) {
                ErrorLogger.LogException("MultipointUtils", "FilterPoints",
                    exc);
    ...
    public static PixelsToLatLong(pts: Array<POINT2>, converter: IPointConversion): Array<POINT2> {
        ...
        for (j = 0; j < n; j++) {
            pt = pts[j];
            ptGeo = MultipointUtils.PointPixelsToLatLong(pt, converter);
            ptsGeo.push(ptGeo);
        }
        return ptsGeo;
    }
    ...
    static LatLongToPixels(pts: Array<POINT2>, converter: IPointConversion): Array<POINT2> {
        ...
        for (j = 0; j < n; j++) {
            pt = pts[j];
            ptPixels = MultipointUtils.PointLatLongToPixels(pt, converter);
    ...
    private static PointLatLongToPixels(ptLatLong: POINT2, converter: IPointConversion): POINT2 {
        ...
        let pt2d: Point2D = MultipointUtils.POINT2ToPoint2D(ptLatLong);
    ...
    static FilterAXADPoints(tg: TacticalGraphic, converter: IPointConversion): void {
        try {
            let lineType: number = tg.lineType;
            switch (lineType) {
                case TacticalLines.CATK:
            ...
            let relativeDist: number = LineUtility.calcDistance(pt0Relative, controlPt);
            relativeDist += 5;
            let pt0pt1dist: number = LineUtility.calcDistance(pt0, pt1);
            ...
            if (relativeDist > pt0pt1dist) {
                ...
                pt1 = LineUtility.extendAlongLine(pt0, pt1, relativeDist);
                pts.push(pt1);
            ...
            if (foundGoodPoint === false) {
                ...
                for (j = 2; j < n - 1; j++) {
                    ...
                    else    //no good points yet
                    {
                        ...
                        pt0pt1dist = LineUtility.calcDistance(pt0, ptj);
                        if (relativeDist > pt0pt1dist) {
            ...
            for (j = 2; j < n - 1; j++) {
                ...
                dist = LineUtility.calcDistance(currentPt, lastGoodPt);
                if (dist > 5) {
                    lastGoodPt = currentPt;
            ...
            if (exc instanceof Error) {
                ErrorLogger.LogException("MultipointUtils", "FilterAXADPoints",
                    exc);
     ...
     * @param tg
     */
    static RemoveDuplicatePoints(tg: TacticalGraphic): void {
        try {
            ...
            switch (tg.lineType)   //preserve segment data
            {
                case TacticalLines.CATK:
                case TacticalLines.AIRAOA:
                case TacticalLines.AAAAA:
            ...
            let ptLast: POINT2;
            let isClosedPolygon: boolean = clsUtilityJTR.isClosedPolygon(tg.lineType);
            let minSize: number = 2;
            if (isClosedPolygon) {
                minSize = 3;
            }

            for (let j: number = 1; j < tg.Pixels.length; j++) {
            ...
            if (exc instanceof Error) {
                ErrorLogger.LogException("MultipointUtils", "RemoveDuplicatePoints",
                    exc);
    ...
    static PointPixelsToLatLong(ptPixels: POINT2, converter: IPointConversion): POINT2 {
        ...
        try {
            ...
            if (exc instanceof Error) {
                ErrorLogger.LogException("MultipointUtils", "PointPixelsToLatLong",
                    exc);
            } else {
                throw exc;
            }
    ...
    static getMBR(clipBounds: Array<Point2D>): Rectangle2D {
        ...
        try {
            let j: number = 0;
            let pt: Point2D;
            let xmax: number = clipBounds[0].getX();
            let xmin: number = xmax;
            let ymax: number = clipBounds[0].getY();
            ...
            if (exc instanceof Error) {
                ErrorLogger.LogException(MultipointUtils._className, "AddBoundaryPointsForLines",
                    exc);
            } else {
                throw exc;
            }
        ...
        try {
            ...
            for (let shape of shapes) {
                ...
                for (let j: number = 0; j < points.length; j++) {
                    let x: number = points[j].x;
                    let y: number = points[j].y;
                    if (x < ptLl.x) {
                        ptLl.x = x;
                        ptUl.x = x;
                    }
            ...
            if (exc instanceof Error) {
                ErrorLogger.LogException(MultipointUtils._className, "GetMBR",
                    exc);
            } else {
                throw exc;
            }
