...
export class countsupport {
    ...
    private static readonly minLength: number = 2.5;		//min arrow size was 5
     ...
     */
    static GetCountersDouble(tg: TacticalGraphic,
        vblCounter: number,
        pLinePoints: POINT2[],
        ...
        try {
            ...
            //declaration section
            let j: number = 0;
            let vblSaveCounter: number = vblCounter;
            ...
            switch (vbiDrawThis) {
                ...
                case TacticalLines.REEF: {
                    vblCounter = countsupport.GetReefCount(pLinePoints, arraysupport.getScaledSize(40, tg.lineThickness, tg.patternScale), vblSaveCounter);
                    count = vblCounter;
                    break;
                }

                case TacticalLines.RESTRICTED_AREA: {
                    vblCounter = countsupport.GetRestrictedAreaCount(pLinePoints, arraysupport.getScaledSize(15, tg.lineThickness, tg.patternScale), vblSaveCounter);
                    count = vblCounter;
                    break;
                }
                ...
                case TacticalLines.PIPE: {
                    count = countsupport.GetPipeCount(pLinePoints, arraysupport.getScaledSize(20, tg.lineThickness, tg.patternScale), vblSaveCounter);
                    break;
                }

                case TacticalLines.ANCHORAGE_AREA:
                case TacticalLines.ANCHORAGE_LINE: {
                    count = flot.GetAnchorageCountDouble(pLinePoints, arraysupport.getScaledSize(20, tg.lineThickness, tg.patternScale), vblCounter);
                    break;
                }

                case TacticalLines.LRO: {
                    let xCount: number = countsupport.GetXPointsCount(pLinePoints, arraysupport.getScaledSize(30, tg.lineThickness, tg.patternScale), vblCounter);
                    let lvoCount: number = countsupport.GetLVOCount(pLinePoints, arraysupport.getScaledSize(30, tg.lineThickness, tg.patternScale), vblCounter);
                    count = xCount + lvoCount;
                    break;
                }

                case TacticalLines.LVO: {
                    count = countsupport.GetLVOCount(pLinePoints, arraysupport.getScaledSize(20, tg.lineThickness, tg.patternScale), vblCounter);
                    break;
                }

                case TacticalLines.ICING: {
                    vblCounter = countsupport.GetIcingCount(pLinePoints, arraysupport.getScaledSize(15, tg.lineThickness, tg.patternScale), vblCounter);
                    count = vblCounter;
                    break;
                }
                ...
                case TacticalLines.FLOT: {
                    ...
                    vblCounter = flot.GetFlotCountDouble(pLinePoints, arraysupport.getScaledSize(20, tg.lineThickness, tg.patternScale), vblCounter);
                    count = vblCounter;
                    break;
                }
                ...
                case TacticalLines.ITD: {
                    vblCounter = countsupport.GetITDQty(pLinePoints, arraysupport.getScaledSize(15, tg.lineThickness, tg.patternScale), vblCounter) + vblCounter;
                    count = vblCounter;
                    break;
                }

                case TacticalLines.CONVERGENCE: {
                    vblCounter = countsupport.GetConvergenceQty(pLinePoints, arraysupport.getScaledSize(10, tg.lineThickness, tg.patternScale), vblCounter) + vblCounter;
                    count = vblCounter;
                    break;
                }
                ...
                case TacticalLines.SHEAR: {
                    vblCounter = countsupport.GetSquallQty(pLinePoints, 6, arraysupport.getScaledSize(30, tg.lineThickness, tg.patternScale), vblSaveCounter as number);
                    count = vblCounter;
                    break;
                }

                case TacticalLines.CABLE: {
                    vblCounter = countsupport.GetSquallQty(pLinePoints, 6, arraysupport.getScaledSize(20, tg.lineThickness, tg.patternScale), vblSaveCounter as number);
                    count = vblCounter;
                    break;
                }

                case TacticalLines.SQUALL: {
                    vblCounter = countsupport.GetSquallQty(pLinePoints, 5, arraysupport.getScaledSize(30, tg.lineThickness, tg.patternScale), vblSaveCounter as number) + 2 * vblSaveCounter;
                    count = vblCounter;
                    break;
                }
                ...
                case TacticalLines.OFY: {
                    vblSaveCounter = vblCounter;
                    vblCounter = flot.GetOFYCountDouble(pLinePoints, arraysupport.getScaledSize(80, tg.lineThickness, tg.patternScale), vblCounter);
                    count = vblCounter;
                    break;
                }
                ...
                case TacticalLines.FORDIF: {
                    dRadius = LineUtility.calcDistanceToLine(pLinePoints[0], pLinePoints[1], pLinePoints[2]);
                ...
                case TacticalLines.LC: {
                    pUpperLinePoints = Channels.GetChannelArray2Double(1, pUpperLinePoints, 1, vblCounter, vbiDrawThis, Math.trunc(arraysupport.getScaledSize(20, tg.lineThickness, tg.patternScale)));
                    pLowerLinePoints = Channels.GetChannelArray2Double(1, pLowerLinePoints, 0, vblCounter, vbiDrawThis, Math.trunc(arraysupport.getScaledSize(20, tg.lineThickness, tg.patternScale)));
                    let lUpperFlotCount: number = flot.GetFlotCountDouble(pUpperLinePoints, arraysupport.getScaledSize(20, tg.lineThickness, tg.patternScale), vblCounter);
                    let lLowerFlotCount: number = flot.GetFlotCountDouble(pLowerLinePoints, arraysupport.getScaledSize(20, tg.lineThickness, tg.patternScale), vblCounter);
    ...
    static GetXPointsCount(pOriginalLinePoints: POINT2[], segmentLength: number, vblCounter: number): number {
    ...
    static GetLVOCount(pOriginalLinePoints: POINT2[], segmentLength: number, vblCounter: number): number {
    ...
    private static GetIcingCount(points: POINT2[], length: number, vblCounter: number): number {
        let total: number = 2 * vblCounter;
        try {
            let d: number = 0;
            for (let j: number = 0; j < vblCounter - 1; j++) {
                d = LineUtility.calcDistance(points[j], points[j + 1]);
                d = (d / length) * 4;
                total += d;
            }
        } catch (exc) {
    ...
    protected static GetITDQty(pLinePoints: POINT2[], length: number, vblCounter: number): number {
        let total: number = 0;
        try {
            let j: number = 0;
            let d: number = 0;
    ...
    protected static GetConvergenceQty(pLinePoints: POINT2[], length: number, vblCounter: number): number {
        let total: number = vblCounter;
        try {
            let j: number = 0;
            let d: number = 0;
            for (j = 0; j < vblCounter - 1; j++) {
    ...
    private static GetDitchCountDouble(pLinePoints: POINT2[],
        vblCounter: number,
        vbiDrawThis: number): number {
        let vblXCounter: number = 0;
        try {
            //declarations
            let j: number = 0;
            let nHowManyThisSegment: number = 0;
            let dHowFar: number = 0;
            //dPrinter = (double) nPrinter;

            vblXCounter = vblCounter;

            for (j = 0; j < vblCounter - 1; j++) {
                dHowFar = LineUtility.calcDistance(pLinePoints[j], pLinePoints[j + 1]);
    ...
    static GetSquallQty(pLinePoints: POINT2[],
        ...
        try {
            let j: number = 0;
            let dist: number = 0;
            let numCurves: number = 0;
            //end declarations

            for (j = 0; j < numPoints - 1; j++) {
                dist = LineUtility.calcDistance(pLinePoints[j], pLinePoints[j + 1]);
                numCurves = Math.trunc(dist / length as number);
     ...
     * @return
     */
    static GetFORTLCountDouble(tg: TacticalGraphic, pLinePoints: POINT2[], vblCounter: number): number {
        ...
        try {
            ...
            for (j = 0; j < vblCounter - 1; j++) {
                dCounter = LineUtility.calcDistance(pLinePoints[j], pLinePoints[j + 1]);
