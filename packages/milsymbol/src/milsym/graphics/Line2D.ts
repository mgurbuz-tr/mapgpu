...
import { type long } from "./BasicTypes";
...
export class Line2D {
    ...
    public static linesIntersect(x1: number, y1: number,
        x2: number, y2: number,
        x3: number, y3: number,
     ...
     * @since 1.2
     */
    public static relativeCCW(x1: number, y1: number, x2: number, y2: number,
        px: number, py: number): number {
        x2 -= x1;
        y2 -= y1;
        px -= x1;
        py -= y1;
        let ccw: number = px * y2 - py * x2;
        if (ccw === 0.0) {
     ...
     * @since 1.2
     */
    public static ptLineDistSq(x1: number, y1: number, x2: number,
        y2: number, px: number, py: number): number {
        ...
        px -= x1;
        py -= y1;
        let dotprod: number = px * x2 + py * y2;
        // dotprod is the length of the px,py vector
        // projected on the x1,y1=>x2,y2 vector times the
        // length of the x1,y1=>x2,y2 vector
        let projlenSq: number = dotprod * dotprod / (x2 * x2 + y2 * y2);
        // Distance to line is now the length of the relative point
        // vector minus the length of its projection onto the line
        let lenSq: number = px * px + py * py - projlenSq;
        if (lenSq < 0) {
            lenSq = 0;
        }
        return lenSq;
     ...
     * @since 1.2
     */
    public static ptLineDist(x1: number, y1: number, x2: number, y2: number,
        px: number, py: number): number {
     ...
     * @since 1.2
     */
    public ptLineDistSq(pt: Point2D): number;
    ...
    public ptLineDistSq(...args: unknown[]): number {
        switch (args.length) {
            case 1: {
     ...
     * @since 1.2
     */
    public ptLineDist(pt: Point2D): number;
    ...
    public ptLineDist(...args: unknown[]): number {
        switch (args.length) {
            case 1: {
                const [pt] = args as [Point2D];
