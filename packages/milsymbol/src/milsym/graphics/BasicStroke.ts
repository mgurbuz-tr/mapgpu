...
export class BasicStroke implements Stroke {
     ...
     * line width.
     */
    public static readonly CAP_SQUARE: number = 2;
    protected width: number;
    protected join: number = 0;
    protected cap: number = 0;
    protected miterlimit: number;
    protected dash: number[] | null;
    protected dash_phase: number;
    ...
    public constructor(width: number, cap: number, join: number);

    public constructor(width: number, cap: number, join: number, miterlimit: number);

    public constructor(width: number, cap: number, join: number, miterlimit: number, dash: number[] | null, dash_phase: number);

    public constructor(width: number = 1.0, cap: number = BasicStroke.CAP_SQUARE, join: number = BasicStroke.JOIN_MITER, miterlimit: number = 10, dash: number[] | null = null, dash_phase: number = 0) {
        if (width < 0.0) {
    ...
    public static GetInteriorPoints(pLinePoints: POINT2[],
        ...
        for (j = 0; j < vblCounter; j++) {
            ...
            m01Val = LineUtility.calcTrueSlope2(pt00, pt01).slope;
            m12Val = LineUtility.calcTrueSlope2(pt10, pt11).slope;
            if (m01Val === m12Val) {
                intersectPt = new POINT2(pt01);
                intersectPoints.push(intersectPt);
                continue;
