...
export class Shape2SVG {
    ...
    public static Convert(text: string, x: number, y: number, font: Font, frc: FontRenderContext, stroke: string, fill: string, strokeWidth: string, strokeOpacity: string, fillOpacity: string, dashArray: string): string;
    public static Convert(...args: unknown[]): string | null {
        switch (args.length) {
            ...
            case 7: {
                ...
                let res: string = "";
                if (textInfo != null) {
                    let name: string = rendererSettings.getLabelFont().getName();
                    if(name !== "sans-serif")
                        name += ", sans-serif";
                    let size: string = rendererSettings.getLabelFont().getSize().toString();
                    let weight: string;
                    ...
                    if (rendererSettings.getLabelFont().isBold()) {
            ...
            case 11: {
                const [text, x, y, font, frc, stroke, fill, strokeWidth, strokeOpacity, fillOpacity, dashArray] = args as [string, number, number, Font, OffscreenCanvasRenderingContext2D, string, string, string, string, string, string];
    ...
    private static convertPath(path2D: Path2D, stroke: string, fill: string, strokeWidth: string, strokeOpacity: string, fillOpacity: string, dashArray: string): string 
    {
        ...
        let line: string = "";
        let moveTo: Point2D;
        let windingRule: number = IPathIterator.WIND_EVEN_ODD;
        let format: number = 1;

        let pitr: PathIterator = path2D.getPathIterator(null);
        windingRule = pitr.getWindingRule();
        do {
            let type: number = pitr.currentSegment(coords);
