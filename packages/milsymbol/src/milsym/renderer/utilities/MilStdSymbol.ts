...
export class MilStdSymbol {
    ...
    protected _Rotation: number = 0.0;//DEGREES
    ...
    protected _patternFillType: number = 0;

    private patternScale: number = 1;
    ...
    public constructor(symbolID: string, uniqueID: string, Coordinates: Array<Point2D>, modifiers: Map<string, string>, keepUnitRatio: boolean = true) {
        ...
        MilStdSymbol._DrawAffiliationModifierAsLabel = rendererSettings.getDrawAffiliationModifierAsLabel();

        MilStdSymbol._UseLineInterpolation = rendererSettings.getUseLineInterpolation();
     ...
     * @deprecated
     */
    public getPatternFillType(): number {
     ...
     * @deprecated
     */
    public setPatternFillType(value: number): void {
    ...
    public setUnitSize(pixelSize: number): void { this._UnitSize = pixelSize; }

    public getUnitSize(): number { return this._UnitSize; }
     ...
     * @return {@link String}
     */
    public getModifier(modifier: string, index: number): string;
    public getModifier(...args: unknown[]): string | null {
        switch (args.length) {
            ...
            case 2: {
                const [modifier, index] = args as [string, number];
     ...
     */
    public setModifier(modifier: string, value: string, index: number): void;
    public setModifier(...args: unknown[]): void {
        switch (args.length) {
            ...
            case 3: {
                const [modifier, value, index] = args as [string, string, number];
     ...
     * @return {@link Double}
     */
    public getModifier_AM_AN_X(modifier: string, index: number): number | null {
    ...
    public setModifier_AM_AN_X(modifier: string, value: number, index: number): void {
        if ((modifier === (Modifiers.AM_DISTANCE)
            || modifier === (Modifiers.AN_AZIMUTH)
            || modifier === (Modifiers.X_ALTITUDE_DEPTH))) {
            let modifiers: Array<number>;
    ...
    */
    public setTextColor(value: Color): void {
        ...
        if(rendererSettings.getTextBackgroundMethod() != RendererSettings.TextBackgroundMethod_NONE && this._TextBackgroundColor == null)
        {
    ...
    */
    ...
    public isTextScaleSensitive():number
    {
        ...
        {
            let msi:MSInfo = msLookup.getMSLInfo(this._symbolID);
            if(msi != null)
            {
    ...
    */
     ...
     * @param scale
     */
    public setPatternScale(scale: number): void {
