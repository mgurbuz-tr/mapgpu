...
import { SinglePointSVGRenderer, singlePointSVGRenderer } from "./SinglePointSVGRenderer"
...
/* implements IIconRenderer */ {
    ...
    private init(): void// List<Typeface> fonts, List<String> xml
    {
        try {
            if (!this._initSuccess) {
                ...
                //test SVGLookup////////////////////////////////////////////////////////////////////
                /*SVGInfo oct = svgLookup.getSVGLInfo("octagon");
                console.log(oct.toString());*/

                //test MSLookup/////////////////////////////////////////////////////////////////////
                /*MSInfo msi = msLookup.getMSLInfo("50110100",0);//
                msi = msLookup.getMSLInfo("36190100",0);//"Non-Mine Mine–Like Object, Bottom"
                console.log(msi.getPath());
                console.log(msi.getName());
                msi = msLookup.getMSLInfo("01110300",0);//"Unmanned Aircraft (UA) / Unmanned Aerial Vehicle (UAV) / Unmanned Aircraft System (UAS) / Remotely Piloted Vehicle (RPV)"
                console.log(msi.getPath());
                console.log(msi.getName());//*/

                // setup single point renderer
                //this._SPR = SinglePointRenderer.getInstance();
                this._SPSVGR = singlePointSVGRenderer;
            ...
            */
            ...
            if (msi == null) {
                ...
                if (msi.getDrawRule() === DrawRules.DONOTDRAW) {
                    ...
                    let si: SVGInfo = svgLookup.getSVGLInfo(lookupSVGID, version);
                    if (si != null)// || (SymbolID.getEntityCode(symbolID)==000000 && svgLookup.getSVGLInfo(SVGLookup.getFrameID(symbolID)) != null))
                    {
                        return true;
                    }
                    else {
    ...
    public AddCustomSymbol(msInfo:MSInfo,svgInfo:SVGInfo):boolean
    {
        ...
        if(msInfo.getBasicSymbolID()===svgInfo.getID())//Make sure IDs match
        {
            ...
            if(msLookup.getMSLInfo(msInfo.getBasicSymbolID(),msInfo.getVersion())==null &&
                svgLookup.getSVGLInfo(svgInfo.getID(),msInfo.getVersion())==null)
                {
                    if(msLookup.addCustomSymbol(msInfo))
                        success = svgLookup.addCustomSymbol(svgInfo,msInfo.getVersion());
                }
