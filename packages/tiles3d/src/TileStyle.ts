...
export class TileStyle {
  private _definition: TileStyleDefinition;
...
function _substituteProps(expr: string, props: Record<string, unknown>): string {
  return expr.replace(/\$\{(\w+)\}/g, (_match, key: string) => {
    const val = props[key];
...
function _evaluateBoolExpr(expr: string, props: Record<string, unknown>): boolean {
  ...
  const compMatch = substituted.match(/^(.+?)\s*(===|!==|>=|<=|>|<)\s*(.+)$/);
  if (compMatch) {
...
function _parseColorExpr(expr: string, props: Record<string, unknown>): [number, number, number, number] {
  ...
  const colorFnMatch = substituted.match(/color\(\s*"([^"]+)"\s*\)/);
  if (colorFnMatch) return _namedColor(colorFnMatch[1]!);

  // rgba(r, g, b, a) — values 0-255
  const rgbaMatch = substituted.match(/rgba\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*([\d.]+)\s*\)/);
  if (rgbaMatch) {
    return [
      Number(rgbaMatch[1]),
      Number(rgbaMatch[2]),
  ...
  const rgbMatch = substituted.match(/rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/);
  if (rgbMatch) {
    return [Number(rgbMatch[1]), Number(rgbMatch[2]), Number(rgbMatch[3]), 255];
  }

  // #rrggbb hex
  const hexMatch = substituted.match(/#([0-9a-fA-F]{6})/);
  if (hexMatch) {
