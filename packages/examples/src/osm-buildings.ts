  ...
  for (const endpoint of OVERPASS_ENDPOINTS) {
    try {
      ...
      if (response.status === 429) {
        log(`429 from ${new URL(endpoint).host}, trying next mirror...`, 'info');
        continue;
      }
      ...
      log(`${new URL(endpoint).host}: ${msg}`, 'info');
      continue;
    }
  }
...
async function main() {
  ...
  async function doFetch() {
    ...
    log(`Fetching buildings for bbox [${south.toFixed(4)},${west.toFixed(4)},${north.toFixed(4)},${east.toFixed(4)}]...`, 'info');
    ...
    try {
      ...
      log(`Loaded ${features.length} buildings (${withLevels} with level data)`, 'success');
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
  ...
  (document.getElementById('chk-auto') as HTMLInputElement).addEventListener('change', (e) => {
    ...
    log(`Auto-fetch: ${autoFetch ? 'ON' : 'OFF'}`, 'info');
  });
}

main().catch((err) => log(`FATAL: ${err}`, 'error'));
