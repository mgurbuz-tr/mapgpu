...
function log(msg: string): void {
  ...
  console.log(`[animated-lines] ${msg}`);
}
...
async function main(): Promise<void> {
  log('Initializing animated lines demo...');
  ...
  await view.when();
  log('MapView ready, GPU initialized');
  ...
  function rebuildGeometry(): void {
    ...
    log(`Geometry: ${lineCount} lines, ${indexCount} indices`);
  }
  ...
  // ─── CP1: Geometry + Uniforms debug ───
  {
    const ext = extrudePolylines(
      Array.from({ length: lineCount }, () => generatePath()),
      Array.from({ length: lineCount }, () => randomColor()),
    );
    const fv = new Float32Array(ext.vertexData);
    console.log('[CP1-GEOM]', {
      lineCount,
      vertexCount: ext.vertexCount,
      indexCount: ext.indexCount,
      vertexFirst10: Array.from(fv.slice(0, 10)),
      indexFirst6: Array.from(ext.indexData.slice(0, 6)),
      hasNaN: Array.from(fv.slice(0, Math.min(fv.length, 200))).some(v => !Number.isFinite(v)),
    });
    ...
    u[2] = 200000;
    u[3] = 800000;
    console.log('[CP1-UNIF]', {
      halfWidth: u[0],
      trailSpeed: u[1],
      trailLength: u[2],
      trailCycle: u[3],
    });
    console.log('[CP1-LAYER]', {
      opacity: shaderLayer.opacity,
      blendState: shaderLayer.blendState,
      animated: shaderLayer.animated,
      topology: (shaderLayer as unknown as Record<string, unknown>).topology ?? 'default',
    });
  }

  view.map.add(shaderLayer);
  log(`Animated lines layer added — ${lineCount} polylines`);
  ...
  log('Demo ready — polylines animating!');
}
