...
async function main(): Promise<void> {
  ...
  view.on('error', (err) => log(`Error: ${JSON.stringify(err)}`, true));

  await view.when();
  log(`GPU ready — mode: ${renderEngine.capabilities.mode}`);
  ...
  view.on('frame', ({ fps }) => {
    frameCount++;
    if (frameCount % 60 === 0) {
      log(`frame=${frameCount} fps=${fps.toFixed(1)}`);
    }
  });

  view.on('view-change', ({ center, zoom, pitch, bearing }) => {
    if (frameCount % 30 === 0) {
      log(`center=[${center[0].toFixed(2)},${center[1].toFixed(2)}] z=${zoom.toFixed(1)} p=${pitch.toFixed(0)} b=${bearing.toFixed(0)}`);
    }
  });
  ...
  log('Controls: left-drag=pan, right-drag=pitch/bearing, wheel=zoom');
}

main().catch((err) => log(`Init failed: ${err}`, true));
