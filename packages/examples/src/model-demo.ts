...
window.addEventListener('error', (e) => log(`UNCAUGHT: ${e.message}`, 'error'));
...
async function main() {
  ...
  log(`GLB parsed: ${parsed.primitives.length} primitives, ${totalVerts} verts, ${hasTextures ? 'textured' : 'no textures'}`, 'success');

  // Upload to GPU
  await view.loadModel(MODEL_ID, glbBuffer);
  log('Model uploaded to GPU', 'success');
  ...
  view.map.add(layer);
  log(`Layer added — ${cities.length} instances at scale ${DEFAULT_SCALE}`, 'success');
  ...
  document.getElementById('btn-add')!.addEventListener('click', () => {
    ...
    log(`Added model at [${lon.toFixed(2)}, ${lat.toFixed(2)}]`, 'info');
  });
