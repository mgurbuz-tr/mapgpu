...
type LogLevel = 'info' | 'success' | 'error' | 'warn' | 'measure';
...
log('Measurement tools registered (Point, Distance, Area)', 'success');
...
view.toolManager.on('tool-activate', ({ toolId }) => {
  updateToolUI(toolId);
  log(`Activated: ${TOOL_LABELS[toolId] ?? toolId}`, 'info');
});
...
view.toolManager.on('measure-complete', (e) => {
  ...
  const val = formatResultValue(entry);
  log(`${e.type.toUpperCase()}: ${val}`, 'measure');
});

view.toolManager.on('measure-clear', ({ toolId }) => {
  log(`Cleared measurements (${TOOL_LABELS[toolId] ?? toolId})`, 'warn');
});
...
$btn('btn-clear-all').addEventListener('click', () => {
  ...
  results.length = 0;
  renderResults();
  log('All measurements cleared', 'warn');
});
...
void view.when().then(() => {
  ...
  updateStatePanel();
  renderResults();
  log('Tip: Select a tool from the sidebar, then click on the map to measure.');
});
