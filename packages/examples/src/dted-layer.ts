...
templateInput.addEventListener('input', () => {
  ...
  log(remoteTemplate ? 'Remote DTED template updated' : 'Remote DTED template cleared');
});
...
fileInput.addEventListener('change', async () => {
  ...
  for (const file of [...files]) {
    try {
      ...
      log(`Local DTED loaded: ${file.name}`);
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      log(`Local DTED load failed: ${file.name} (${msg})`);
    }
  }
  terrainLayer.refresh();
});
...
document.getElementById('btn-clear')!.addEventListener('click', () => {
  terrainLayer.refresh();
  log('Terrain tile cache cleared');
});
...
void view.when().then(() => {
  log('DTED demo ready');
  log('Tip: Add local .dt0/.dt1/.dt2 files first, then zoom in for detail.');
});
