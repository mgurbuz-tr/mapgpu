...
log('MapView created — center: [29.0, 41.0], zoom: 6');
...
btnGrid.addEventListener('click', () => {
  ...
  log(gridOn ? 'Tile grid: ON' : 'Tile grid: OFF');
});
...
btnBrush.addEventListener('click', () => {
  ...
  log(brushEnabled ? 'Height brush: ON — click/drag to raise terrain' : 'Height brush: OFF');
  const radiusSpan = document.getElementById('info-radius');
