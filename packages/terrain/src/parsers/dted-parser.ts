...
export function extractCoordsFromFilename(fileName: string): [number, number] | null {
  ...
  const flatMatch = normalized.match(/([ns])(\d{2})([ew])(\d{3})/);
  if (flatMatch) {
    let lat = parseInt(flatMatch[2] ?? '0', 10);
    let lon = parseInt(flatMatch[4] ?? '0', 10);
    if (flatMatch[1] === 's') lat = -lat;
    if (flatMatch[3] === 'w') lon = -lon;
    return [lon, lat];
  }
