...
import type { LosAnalysis } from '@mapgpu/analysis';
...
/**
 * Globe vector pipelines apply ALTITUDE_EXAG = 5.0 (h / R * 5),
 * but extrusion pipeline (buildings) uses raw h / R.
 * Divide LOS Z by 5 so visual height matches building scale.
 *
 * Example: observer at 1.8m, building at 20m
 *  - Without: 1.8 / R * 5 = 9m equivalent → looks half a 20m building (wrong)
 *  - With:    0.36 / R * 5 = 1.8m equivalent → correct proportion to buildings
 */
const EXTRUSION_SCALE_MATCH = 1 / 5;

export class LosTool extends ToolBase {
  readonly id = 'los';
  readonly name = 'Line of Sight';

  private _analysis: LosAnalysis;
  ...
  private _updatePreview(): void {
    ...
    // Scale Z to match extrusion pipeline (buildings) — compensate for 5x ALTITUDE_EXAG
    const obsZ = this._observerOffset * EXTRUSION_SCALE_MATCH;
    const tgtZ = this._targetOffset * EXTRUSION_SCALE_MATCH;
