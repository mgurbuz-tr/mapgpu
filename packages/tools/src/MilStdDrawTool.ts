...
import type { ToolPointerEvent, Feature, Geometry } from '@mapgpu/core';
...
export class MilStdDrawTool extends ToolBase {
  ...
  private _finishTacticalGraphic(): void {
    ...
    const geomType: Geometry['type'] = this._vertices.length === 1 ? 'Point'
      : this._vertices.length === 2 ? 'LineString'
      : 'Polygon';
