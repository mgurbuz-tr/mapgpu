...
import type { ToolPointerEvent, ToolContext, Feature, ICommand, Geometry } from '@mapgpu/core';
...
class GeometryEditCommand implements ICommand {
  ...
  private _oldGeom: string; // JSON snapshot
  private _newGeom: string;

  constructor(
    private _layer: ITargetLayer,
...
export class EditTool extends ToolBase {
  ...
  private _editableLayers: EditToolOptions['editableLayers'];
  private _hitTolerance: number;
    ...
    if (this._state === 'editing' && this._selectedFeature) {
      ...
      if (this._editableVertices.length > 0) {
        ...
        if (hit) {
          ...
          this._dragGeomSnapshot = JSON.parse(JSON.stringify(this._selectedFeature.geometry)) as Geometry;
      ...
      if (this._editableVertices.length >= 2) {
        ...
        if (midHit !== null) {
          const oldGeom = JSON.parse(JSON.stringify(this._selectedFeature.geometry)) as Geometry;
          const mp = midHit.coords;
          this._editableVertices.splice(midHit.insertIndex, 0, mp);
          this._writeVerticesToGeometry();

          const newGeom = JSON.parse(JSON.stringify(this._selectedFeature.geometry)) as Geometry;
          const layer = this._findLayer(this._selectedLayerId);
          if (layer) {
            this._selectedFeature.geometry = JSON.parse(JSON.stringify(oldGeom)) as Geometry;
  ...
  onPointerUp(e: ToolPointerEvent): boolean {
    ...
    if (this._draggingVertexIndex !== null && this._dragGeomSnapshot) {
      if (this._isDragging && this._selectedFeature) {
        ...
        if (layer) {
          this._selectedFeature.geometry = JSON.parse(JSON.stringify(this._dragGeomSnapshot)) as Geometry;
          const cmd = new GeometryEditCommand(
   ...
   */
  private _featureScreenDistance(
    feature: Feature,
    screenX: number,
    screenY: number,
  ...
  private _hitTestMidpoints(
    ...
    if (this._isClosedRing && this._editableVertices.length >= 3) {
      ...
      const last = this._editableVertices[this._editableVertices.length - 1]!;
