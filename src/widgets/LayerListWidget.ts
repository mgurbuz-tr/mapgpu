...
import type { ILayer, IView, WidgetPosition } from '../core/index.js';
...
export class LayerListWidget extends WidgetBase {
  ...
  private readonly _listeners = new Map<keyof LayerListEvents, Set<LayerListEventHandler<keyof LayerListEvents>>>();
  ...
  private _subscribeLayerEvents(layer: ILayer): void {
    const onVisChange = (_visible: boolean): void => {
      this._updateLayerItem(layer);
    };
    const onOpacityChange = (_opacity: number): void => {
      this._updateLayerItem(layer);
    };
  ...
  private _rebuildList(): void {
    ...
    for (const layer of this._layers) {
      ...
      slider.addEventListener('input', () => {
        layer.opacity = Number.parseInt(slider.value, 10) / 100;
      });
