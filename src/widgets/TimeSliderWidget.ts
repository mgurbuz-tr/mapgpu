...
export class TimeSliderWidget extends WidgetBase {
  ...
  private readonly _timeChangeHandlers = new Set<(date: Date) => void>();

  constructor(options: TimeSliderWidgetOptions) {
    super('timeslider', options);
    this._min = options.min;
    this._max = options.max;
    this._value = options.value ?? new Date(+options.min);
  ...
  stop(): void {
    this.pause();
    this._value = new Date(+this._min);
    this._updateSlider();
    this._emitTimeChange();
  }
  ...
  protected render(root: HTMLElement): void {
    ...
    this._sliderEl.addEventListener('input', () => {
      const time = Number.parseInt(this._sliderEl!.value, 10);
