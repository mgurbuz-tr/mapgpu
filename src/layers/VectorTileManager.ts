...
import { createVectorTileTaskDef } from './vector-tile-worker-protocol.js';
...
export class VectorTileManager {
  ...
  private readonly _maxEntries = DEFAULT_CACHE_ENTRIES;
  ...
  /**
   * Type-safe descriptor for the vector-tile worker task. Built once, reused
   * across every dispatch so `pool.run(taskDef, request)` gets full inference.
   */
  private readonly _taskDef: WorkerTaskDef<VectorTileWorkerRequest, VectorTileWorkerResponse> =
    createVectorTileTaskDef(() => {
  ...
  private async _parseTileInWorker(
    ...
    const response = await pool.run(this._taskDef, request);
  ...
  private _ensureWorkerPool(): WorkerPool | null {
    ...
    if (!this._workerPool) {
      this._workerPool = new WorkerPool({
        ...
        workerFactory: this._taskDef.workerFactory,
      });
