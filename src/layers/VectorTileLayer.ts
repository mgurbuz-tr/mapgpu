...
const DEFAULT_UPDATE_CONTEXT: VectorTileUpdateContext = { renderMode: '2d' };
...
export class VectorTileLayer extends LayerBase implements ITileLayer, IFeatureLayer {
  ...
  private readonly _vtManager: VectorTileManager;
  ...
  private _lastTileCoordSet: Set<string> | null = null;
  ...
  updateVisibleTiles(
    ...
    context: VectorTileUpdateContext = DEFAULT_UPDATE_CONTEXT,
  ): void {
    // Fast change detection: check count first, then Set membership with early exit
    if (
      !this._tilesDirty &&
      this._lastTileCoordSet !== null &&
      coords.length === this._lastTileCoordSet.size
    ) {
      let allMatch = true;
      for (const c of coords) {
        if (!this._lastTileCoordSet.has(`${c.z}/${c.x}/${c.y}`)) {
          allMatch = false;
          break;
        }
      }
    ...
    for (const c of coords) newSet.add(`${c.z}/${c.x}/${c.y}`);
    this._lastTileCoordSet = newSet;
