...
function buildBinaryPayload( // NOSONAR
  features: VectorTileWorkerFeature[],
...
function resolveSymbol( // NOSONAR
...
function extrudePolygon( // NOSONAR
  ...
  for (const ring of projectedRings) {
    for (let i = 0; i < ring.length; i++) {
      ...
      outIndices.push(base, base + 1, base + 2, base, base + 2, base + 3);
...
function symbolKey(sym: VectorRenderableSymbol): string {
  ...
  if (sym.type === 'model') {
    return `M:${sym.modelId}:${sym.scale ?? 1}:${sym.heading ?? 0}:${sym.pitch ?? 0}:${sym.roll ?? 0}:${sym.anchorZ ?? 0}:${sym.tintColor ?? ''}:${sym.outlineColor ?? ''}:${sym.outlineWidth ?? 0}`;
  }
...
function derivePointSymbol(sym: VectorRenderableSymbol): PointSymbol {
  ...
  return {
    type: 'simple-marker',
    color: c,
    size: 'size' in sym ? sym.size : 10,
    outlineColor: outline,
    outlineWidth: 'outlineWidth' in sym ? sym.outlineWidth : 1,
  };
}
