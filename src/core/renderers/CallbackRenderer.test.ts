...
describe('CallbackRenderer', () => {
  ...
  it('passes context through to the callback', () => {
    ...
    const context: SymbolRenderContext = { renderMode: '3d', zoom: 10, resolution: 152.87 };
  ...
  it('supports zoom-dependent symbology via context', () => {
    ...
    const lowZoom = renderer.getSymbol(feature, { renderMode: '2d', zoom: 3, resolution: 1000 });
    const highZoom = renderer.getSymbol(feature, { renderMode: '3d', zoom: 15, resolution: 10 });
  ...
  it('supports mode-aware symbology via context.renderMode', () => {
    const renderer = new CallbackRenderer((_feature, context) => (
      context?.renderMode === '3d' ? blueLine : redMarker
    ));
    const feature = makeFeature('f1');

    expect(renderer.getSymbol(feature, { renderMode: '2d', zoom: 8, resolution: 50 })).toBe(redMarker);
    expect(renderer.getSymbol(feature, { renderMode: '3d', zoom: 8, resolution: 50 })).toBe(blueLine);
  });
});
