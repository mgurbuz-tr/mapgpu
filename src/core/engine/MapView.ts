...
import {
  ...
  type DepthConfig,
  type GlobeEffectsConfig,
  type ModelBoundsQuery,
  type ModelMetadata,
  type ResolvedModelBounds,
...
// Reverse-Z: near→1, far→0. depth32float'ın mantissa precision'ı 1/z dağılımıyla
// birleştiğinde near/far arasında neredeyse uniform depth resolution sağlar. 3D
// globe'da modelin kameraya uzak yarısında (büyük w) adjacent üçgenlerin aynı
// depth bucket'ına düşmesini (z-fighting) çözer.
...
export interface MapViewOptions {
   ...
   *
   * Accepts a CSS color string (`"transparent"`, `"#rrggbb"`, `"#rrggbbaa"`,
   * `"rgb(...)"`, `"rgba(...)"`, named color) or an `[R, G, B, A]` tuple with
   * channels in 0-1.
   *
   * When the resolved alpha is `0` AND `globeEffects.sky.enabled` was not
   * explicitly set, the 3D sky pass is implicitly disabled so the transparent
   * clear reaches the host page. Shorthand for setting
   * `globeEffects: { backgroundColor: ... }` — if both are provided, the
   * nested `globeEffects.backgroundColor` wins.
   *
   * @defaultValue `'black'`
   */
 ...
 * import { WebGPURenderEngine } from '../../render/index.js';
 *
 * const view = new MapView({
 *   container: '#map',
 *   mode: '2d',
...
export class MapView implements IView {
  ...
  private readonly _core: ViewCore;
  private _mode: IViewMode;
  private readonly _events = new EventBus<MapViewEvents>();
  private _ready = false;
  private _destroyed = false;
  private _readyResolve: (() => void) | null = null;
  private readonly _readyPromise: Promise<void>;
  private readonly _interactionOptions: InteractionHandlerOptions | GlobeInteractionOptions | false;
  private readonly _animatedLayerCallbacks = new Map<string, (deltaMs: number, frameNumber: number) => void>();
   ...
   * const headless = new MapView({ container: null }); // NOSONAR
   * ```
   */
  constructor(options: MapViewOptions) { // NOSONAR
    ...
    // Apply globe effects config. `options.backgroundColor` is a convenience
    // shortcut for `globeEffects.backgroundColor`; if the caller supplies both,
    // the nested form wins.
    if (options.globeEffects || options.backgroundColor !== undefined) {
      const mergedGlobeEffects: GlobeEffectsConfig = {
        ...(options.globeEffects ?? {}),
      };
      if (
        options.backgroundColor !== undefined &&
        mergedGlobeEffects.backgroundColor === undefined
      ) {
        mergedGlobeEffects.backgroundColor = options.backgroundColor;
      }
      this._core.globeEffects = resolveGlobeEffects(mergedGlobeEffects);
    }
    ...
    if (options.renderEngine) {
      ...
      // Set clear color from globe effects. Applied in both 2D and 3D so that
      // `"transparent"` and other custom backgrounds work uniformly across modes.
    ...
    if (this._core.container) {
      ...
      const onUp = (e: PointerEvent): void => {
        ...
        if (Math.hypot(dx, dy) < MAX_CLICK_DIST && elapsed < MAX_CLICK_TIME) {
    ...
    // GPU init (async in constructor is intentional — promise-based init pattern)
    if (options.renderEngine && this._core.canvas) {
      this._core.initGpu( // NOSONAR
  ...
  /**
   * The underlying WebGPU device, or `null` before {@link when} resolves.
   * Exposed for advanced use cases (custom WGSL layers that allocate
   * their own GPU buffers / textures). Prefer library abstractions over
   * poking at the device directly when one exists.
   */
  get device(): GPUDevice | null { return this._core.renderEngine?.device ?? null; }
  ...
  async switchTo(mode: '2d' | '3d'): Promise<void> {
    ...
    // Apply mode-specific depth policy. We use rebuildForDepthChange instead
    // of the heavier recover() because:
    //   - 2D uses DEPTH_STANDARD, 3D uses DEPTH_3D_HIGH_PRECISION (reverse-Z);
    //     only the depth texture + pipeline state must change.
    //   - The WebGPU device, buffer pool, tile cache and layer buffers are
    //     projection-agnostic and stay valid across the switch.
    //   - Keeping the tile cache alive preserves the parent-fallback chain,
    //     which eliminates the "flash of null tiles" on mode switch that a
    //     full recover() would produce (cache gets wiped because textures
    //     would otherwise leak across the old → new device boundary).
    if (this._core.gpuReady && this._core.renderEngine) {
      ...
      await this._core.renderEngine.rebuildForDepthChange(depthConfigForMode(mode));
   ...
   * @param screenY - Vertical pixel coordinate relative to the canvas.
   */
  async hitTest(screenX: number, screenY: number): Promise<HitTestResult[]> { // NOSONAR
    if (!this._core.renderEngine) return [];
    ...
    try {
      ...
      try {
        const img = await new Promise<HTMLImageElement>((resolve, reject) => {
          ...
          image.onerror = (e) => reject(new Error(`SVG image load failed for "${id}": ${typeof e === 'object' ? JSON.stringify(e) : String(e)}`));
          image.src = url;
        });
  ...
  private _applyCameraLock(deltaMs = 0): void { // NOSONAR
