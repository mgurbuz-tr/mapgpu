...
import type { IRenderEngine, GpuCapabilities, DepthConfig, ResolvedGlobeEffects } from '../interfaces/index.js';
...
export class ViewCore {
  ...
  /**
   * Shared registry of worker pools, keyed by task type.
   *
   * Each {@link WorkerTaskDef} gets its own lazily-initialized pool. Managers
   * (LosAnalysis, future DTED worker, etc.) call
   * `viewCore.workerRegistry.run(TASK, data)` to dispatch off-main-thread work.
   * The registry is terminated on {@link destroy}.
   *
   * Note: VectorTileManager currently maintains its own pool because its
   * worker count is user-configurable via `setPerformance()`. Other task
   * types flow through the registry.
   *
   * @see {@link WorkerPoolRegistry}
   */
  readonly workerRegistry: WorkerPoolRegistry;
   ...
   */
  globeEffects: ResolvedGlobeEffects = resolveGlobeEffects();
  ...
  constructor() {
    ...
    this.workerRegistry = new WorkerPoolRegistry();
  }
  ...
  async initGpu(
    ...
    // Forward the shared WorkerPoolRegistry to the render engine if the
    // engine supports it. Duck-typed so IRenderEngine can stay minimal —
    // alternative engines (e.g., mock/test) without worker support just
    // skip this step.
    const engineWithRegistry = engine as IRenderEngine & {
      setWorkerRegistry?: (r: WorkerPoolRegistry | null) => void;
    };
    if (typeof engineWithRegistry.setWorkerRegistry === 'function') {
      engineWithRegistry.setWorkerRegistry(this.workerRegistry);
    }
    ...
    this.terrainManager.onDirty = () => {
      this.bufferCache.bumpTerrainVersion();
      this.renderLoop.markDirty();
    };
