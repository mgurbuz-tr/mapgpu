...
describe('WorkerPool', () => {
  ...
  it('should prefer the least-busy worker when one has more pending tasks', async () => {
    const workers: ReturnType<typeof createMockWorker>[] = [];
    const pool = new WorkerPool({
      maxWorkers: 2,
      workerFactory: () => {
    ...
    // Dispatch 3 tasks but resolve the second one so worker[1] becomes idle.
    const p1 = pool.dispatch('a', {}); // w0 (pending=1)
    const p2 = pool.dispatch('b', {}); // w1 (pending=1)
    const id2 = workers[1]!.messages[0]!.id;
    workers[1]!.simulateResponse({ id: id2, result: 'ok' });
    await p2;

    // Now w0=1, w1=0 → next dispatch should go to w1.
    const p3 = pool.dispatch('c', {});
    expect(workers[1]!.messages.length).toBe(2);
    expect(workers[0]!.messages.length).toBe(1);

    // Clean up.
    workers[0]!.simulateResponse({ id: workers[0]!.messages[0]!.id, result: 'ok' });
    workers[1]!.simulateResponse({ id: workers[1]!.messages[1]!.id, result: 'ok' });
  ...
  it('run() dispatches via a WorkerTaskDef with inferred types', async () => {
    interface ComputeReq { x: number; y: number; }
    interface ComputeRes { sum: number; }

    const COMPUTE_TASK: WorkerTaskDef<ComputeReq, ComputeRes> = {
      taskType: 'math:add',
      workerFactory: createMockWorker,
    };
    ...
    const promise = pool.run(COMPUTE_TASK, { x: 1, y: 2 });
    const msg = workers[0]!.messages[0]!;
    expect(msg.type).toBe('math:add');
    expect(msg.data).toEqual({ x: 1, y: 2 });

    workers[0]!.simulateResponse({ id: msg.id, result: { sum: 3 } });
    const result = await promise;
  ...
  it('run() collects transferables from the task def', async () => {
    const buf = new ArrayBuffer(8);
    interface XferReq { buffer: ArrayBuffer; }
    const XFER_TASK: WorkerTaskDef<XferReq, string> = {
      taskType: 'xfer',
      workerFactory: createMockWorker,
    ...
    const promise = pool.run(XFER_TASK, { buffer: buf });
    expect(workers[0]!.postMessage).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'xfer' }),
      [buf],
    );
