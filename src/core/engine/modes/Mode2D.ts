...
export class Mode2D implements IViewMode {
  ...
  renderFrame(ctx: RenderFrameContext): void { // NOSONAR
    ...
    // MUST run before any delegate opens the scene render pass on the frame
    // command encoder — WebGPU forbids opening a compute pass on an encoder
    // locked by an active render pass. Wall-clock dt keeps the simulation
    ...
    for (const layerId of overlayLayerIds) {
      renderOverlayLayer(layerId, renderEngine, layerManager, this._overlayTextureCache, false);
    }
    ...
    // dispatch already ran in the pre-pass phase above, so this call is
    // safe to issue while the scene render pass is open.
    for (const layerId of particleLayerIds) {
      renderParticleLayer(layerId, renderEngine, layerManager, false);
    }
  }
