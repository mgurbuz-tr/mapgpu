...
function makeWallLayer(): ILayer & {
  ...
  return {
    ...
    loaded: true,
    interactive: false,
    renderer: makeSimpleRenderer(RED_FILL),
    getWallGeometryData: () => ({
      positions: [[29, 41], [30, 41], [30, 42]],
      maximumHeights: [500, 600, 700],
...
const TERRAIN_RELATIVE: ElevationInfo = { mode: 'relative-to-ground' };
...
describe('VectorBufferCache', () => {
  ...
  describe('getOrBuild', () => {
    ...
    it('routes wall layers to real mesh buffers instead of thin polygon fills', () => {
      const wallLayer = makeWallLayer();
      const features = [makePolygonFeature()];
      const entry = cache.getOrBuild('wall', features, wallLayer.renderer, undefined, wallLayer);

      expect(entry).not.toBeNull();
      expect(entry!.mesh3dGroups).toHaveLength(1);
      expect(entry!.mesh3dGroups[0]!.buffer.indexCount).toBe(12);
      expect(entry!.polygonGroups).toHaveLength(0);
      expect(entry!.lineGroups).toHaveLength(1);
      expect(entry!.lineGroups[0]!.symbol.color).toEqual(RED_FILL.outlineColor);
    });
  ...
  describe('terrain version invalidation', () => {
    it('keeps non-terrain layer entries cached after terrain version bumps', () => {
      const features = [makePointFeature(0, 0)];

      const first = cache.getOrBuild('plain-layer', features);
      cache.bumpTerrainVersion();
      const second = cache.getOrBuild('plain-layer', features);

      expect(second).toBe(first);
      expect(engine.releaseBuffer).not.toHaveBeenCalled();
      expect((engine.createBuffer as ReturnType<typeof vi.fn>).mock.calls).toHaveLength(1);
    });

    it('lazily rebuilds terrain-relative layer entries on the next access', () => {
      const features = [makePointFeature(0, 0)];
      const sampler = { sampleElevation: vi.fn(() => 120) };
    ...
    it('does not drop cached vector-tile entries when terrain version changes', () => {
      const features = [makeProjectedPointFeature(0, 0)];

      const first = cache.getOrBuildTile({
        layerId: 'vt',
        tileKey: '0/0/0',
        version: 1,
        globe: true,
      ...
      const second = cache.getOrBuildTile({
        layerId: 'vt',
        tileKey: '0/0/0',
        version: 1,
        globe: true,
  ...
  describe('terrain sampling memoization', () => {
    it('reuses terrain samples for repeated point coordinates within a build', () => {
      const sampler = { sampleElevation: vi.fn(() => 15) };
      const features = [
        makePointFeature(10, 20),
        makePointFeature(10, 20),
        makePointFeature(10, 20),
      ];
      ...
      expect(sampler.sampleElevation).toHaveBeenCalledTimes(1);
    });

    it('reuses terrain samples for repeated line prev/curr/next coordinates within a build', () => {
      const sampler = { sampleElevation: vi.fn(() => 15) };
      const features = [makeLineFeature(), makeLineFeature()];
      ...
      expect(sampler.sampleElevation).toHaveBeenCalledTimes(3);
    });
  });
...
describe('renderer integration', () => {
  ...
  describe('renderer returning null', () => {
    ...
    it('passes renderMode through so renderers can vary symbol families by view mode', () => {
      const getSymbol = vi.fn((_feature: Feature, context) =>
        context?.renderMode === '3d' ? BLUE_FILL : RED_FILL,
      );
      const renderer: IRenderer = {
        type: 'callback',
        getSymbol,
      ...
      const entry2d = cache.getOrBuild('mode-aware-2d', [makePolygonFeature()], renderer, 8, undefined, '2d');
      const entry3d = cache.getOrBuild('mode-aware-3d', [makePolygonFeature()], renderer, 8, undefined, '3d');
      const renderCalls = getSymbol.mock.calls.filter(([, context]) => context !== undefined);

      expect(entry2d!.polygonGroups[0]!.symbol).toEqual(RED_FILL);
      expect(entry3d!.polygonGroups[0]!.symbol).toEqual(BLUE_FILL);
      expect(renderCalls[0]).toEqual([
        expect.objectContaining({ geometry: expect.objectContaining({ type: 'Polygon' }) }),
        expect.objectContaining({ renderMode: '2d', zoom: 8, resolution: 0 }),
      ]);
      expect(renderCalls[1]).toEqual([
        expect.objectContaining({ geometry: expect.objectContaining({ type: 'Polygon' }) }),
        expect.objectContaining({ renderMode: '3d', zoom: 8, resolution: 0 }),
      ]);
    });
  });
...
describe('Model Symbol Support', () => {
  ...
  it('releases mesh3d buffers on invalidate', () => {
    const wallLayer = makeWallLayer();
    cache.getOrBuild('wall-release', [makePolygonFeature()], wallLayer.renderer, undefined, wallLayer);

    cache.invalidate('wall-release');
    expect(engine.releaseBuffer).toHaveBeenCalledTimes(4);
  });
});
