...
export type FrameCallback = (deltaMs: number, frameNumber: number) => void;
...
export class RenderLoop {
  ...
  private readonly _minFrameInterval: number;

  // readonly FPS smoothing
  ...
  private readonly _frameCallbacks: Set<FrameCallback> = new Set();
  private readonly _preFrameCallbacks: Set<FrameCallback> = new Set();

  // readonly Allow injecting a custom requestAnimationFrame for testing
  private readonly _requestAnimationFrame: (cb: FrameRequestCallback) => number;
  private readonly _cancelAnimationFrame: (id: number) => void;
  ...
  private _tick(timestamp: number): void { // NOSONAR
    if (!this._running) return;
