...
describe('VerticalPerspectiveTransform', () => {
  ...
  it('limits zoom when a minimum camera-surface distance is required', () => {
    const t = new VerticalPerspectiveTransform({
      zoom: 22,
      pitch: 85,
      viewportWidth: 800,
    ...
    t.setMinCameraSurfaceDistance(10);

    expect(t.zoom).toBeLessThan(22);
    expect(t.cameraSurfaceDistanceMeters).toBeGreaterThan(9.999);
  });

  it('setPitch clamps to [0, 85]', () => {
    const t = new VerticalPerspectiveTransform();
    t.setPitch(90);
    expect(t.pitch).toBe(85);
    t.setPitch(-10);
    expect(t.pitch).toBe(0);
  });

  it('re-clamps zoom when pitch increases under a surface-distance constraint', () => {
    const t = new VerticalPerspectiveTransform({
      zoom: 22,
      pitch: 0,
      viewportWidth: 800,
    ...
    t.setMinCameraSurfaceDistance(5);
    const zoomBeforePitch = t.zoom;
    t.setPitch(85);

    expect(t.zoom).toBeLessThan(zoomBeforePitch);
    expect(t.cameraSurfaceDistanceMeters).toBeGreaterThan(4.999);
  });
