...
import { parseColor, type ColorInput } from '../utils/parse-color.js';
...
export interface GlobeEffectsConfig {
  ...
  /**
   * Background (clear) color for 3D mode. Accepts:
   * - CSS color string: `"transparent"`, `"#rrggbb"`, `"#rrggbbaa"`, `"rgb(...)"`, `"rgba(...)"`, or a named color
   * - `[R, G, B, A]` tuple with channels in 0-1
   *
   * When the resolved alpha is `0` AND `sky.enabled` was not explicitly set,
   * the 3D sky pass is implicitly disabled so the transparent clear survives
   * to the host page. Default: `[0, 0, 0, 1]` (opaque black).
   */
...
export function resolveGlobeEffects(cfg?: GlobeEffectsConfig): ResolvedGlobeEffects {
  const backgroundColor: [number, number, number, number] = cfg?.backgroundColor !== undefined
    ? parseColor(cfg.backgroundColor)
    : [0, 0, 0, 1];

  // If the resolved background alpha is zero AND the caller did not explicitly
  // set `sky.enabled`, implicitly disable the sky pass so the transparent
  // clear reaches the host page instead of being overwritten by the sky mesh.
  const userSetSkyEnabled = cfg?.sky?.enabled !== undefined;
  const skyCfg: Partial<SkyConfig> | undefined =
    !userSetSkyEnabled && backgroundColor[3] === 0
      ? { ...(cfg?.sky ?? {}), enabled: false }
      : cfg?.sky;

  const sky = resolveSkyConfig(skyCfg);
  return {
    fog: {
      enabled: cfg?.fog?.enabled ?? false,
      density: cfg?.fog?.density ?? 0.0003,
      color: cfg?.fog?.color ?? [0.6, 0.7, 0.9, 1],
    ...
    atmosphere: {
      ...
      colorInner: cfg?.atmosphere?.colorInner ?? [0.3, 0.5, 1, 0.3],
      colorOuter: cfg?.atmosphere?.colorOuter ?? [0.1, 0.3, 0.8, 0],
      strength: cfg?.atmosphere?.strength ?? 1,
      falloff: cfg?.atmosphere?.falloff ?? 4,
    },
    sky,
    lighting: {
...
function resolveSkyConfig(cfg?: Partial<SkyConfig>): ResolvedSkyConfig {
  ...
  // When a named preset is active, its color palette (horizon / zenith /
  // space) is authoritative — user overrides are ignored for those
  // fields. Otherwise changing `preset` at runtime through
  // `MapView.setGlobeEffects` would leave the previous preset's colors
  // in place because the old values come in via the prev.sky merge.
  // Non-color user overrides (starIntensity, horizonBlend, …) still win.
  const isCustom = preset === 'custom';
  return {
    preset,
    enabled: cfg?.enabled ?? presetDefaults.enabled,
    horizonColor: isCustom ? (cfg?.horizonColor ?? presetDefaults.horizonColor) : presetDefaults.horizonColor,
    zenithColor:  isCustom ? (cfg?.zenithColor  ?? presetDefaults.zenithColor)  : presetDefaults.zenithColor,
    spaceColor:   isCustom ? (cfg?.spaceColor   ?? presetDefaults.spaceColor)   : presetDefaults.spaceColor,
