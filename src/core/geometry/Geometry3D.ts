...
export function createCylinderGeometry(
  ...
  for (let i = 0; i <= segments; i++) {
    ...
    posArr.push(cos * radiusBottom, -halfH, sin * radiusBottom,
                cos * radiusTop, halfH, sin * radiusTop);

    // Normal: approximate for tapered cylinder
    const slope = radiusBottom - radiusTop;
    const len = Math.hypot(slope, height);
    const nx = cos * height / len;
    const ny = slope / len;
    const nz = sin * height / len;
    normArr.push(nx, ny, nz, nx, ny, nz);

    if (i < segments) {
      const base = vi;
      idxArr.push(base, base + 1, base + 3, base, base + 3, base + 2);
      vi += 2;
    }
  }
  vi += 2; // last pair
...
export function createWallGeometry(
  ...
  for (let i = 0; i < positions.length - 1; i++) {
    ...
    posArr.push(
      mx0, my0, mins[i]!,
      mx1, my1, mins[i + 1]!,
      mx1, my1, maximumHeights[i + 1]!,
      mx0, my0, maximumHeights[i]!,
    );

    // Normal in Mercator XY plane (Z=0 for vertical walls)
    normArr.push(nx, ny, 0, nx, ny, 0, nx, ny, 0, nx, ny, 0);
...
export class WallGeometryBuilder {
  private readonly _lons: number[] = [];
  private readonly _lats: number[] = [];
  private readonly _maxH: number[] = [];
  private readonly _minH: number[] = [];
...
export function createCorridorGeometry(
  ...
  for (let i = 0; i < positions.length; i++) {
    posArr.push(leftPoints[i]![0], y, leftPoints[i]![1],
                rightPoints[i]![0], y, rightPoints[i]![1]);
  ...
  if (height > 0) {
    ...
    for (let i = 0; i < positions.length; i++) {
      posArr.push(leftPoints[i]![0], 0, leftPoints[i]![1],
                  rightPoints[i]![0], 0, rightPoints[i]![1]);
      normArr.push(0, -1, 0, 0, -1, 0);
