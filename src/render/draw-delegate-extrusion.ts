...
import type { ExtrusionRenderBuffer, ExtrudedPolygonSymbol } from '../core/index.js';
...
export class DrawDelegateExtrusion {
  ...
  private readonly extrusionMaterials = new Map<string, CachedUniformResource>();
  private readonly globeExtrusionMaterials = new Map<string, CachedUniformResource>();
  /** Tracks animation start per stable id (tileKey) or GPUBuffer fallback */
  private readonly animState = new Map<string, { startTime: number; origin: [number, number] }>();
  /** Tile IDs whose grow animation has completed — never re-animate */
  private readonly animCompleted = new Set<string>();
  ...
  private _createMaterialResource( // NOSONAR
    ...
    const delayFactor = anim?.delayFactor ?? 2;
    ...
    if (effectiveAnimDuration > 0) {
      ...
      if (!state) {
        ...
        if (cam) {
          ...
          waveOriginY = 1 - (cam.position[1] + HALF_C) / (2 * HALF_C);
        }
