 ...
 */
export function gridCluster( // NOSONAR
  points: Float32Array,
...
/**
 * Inverse of {@link packClusterEntries} — rebuild a {@link CpuClusterResult}
 * from the zero-copy transferable representation used by the cluster worker
 * protocol (packed Float32 entries + flat Int32 membership values +
 * membership offsets).
 */
export function unpackClusterResult(
  packedEntries: Float32Array,
  membershipValues: Int32Array,
  membershipOffsets: Int32Array,
): CpuClusterResult {
  const u32View = new Uint32Array(packedEntries.buffer, packedEntries.byteOffset, packedEntries.length);
  const entryCount = packedEntries.length / 4;
  const entries: CpuClusterEntry[] = new Array<CpuClusterEntry>(entryCount);
  const membership: number[][] = new Array<number[]>(entryCount);

  for (let i = 0; i < entryCount; i++) {
    const off = i * 4;
    entries[i] = {
      posX: packedEntries[off]!,
      posY: packedEntries[off + 1]!,
      count: u32View[off + 2]!,
      flags: u32View[off + 3]!,
    };
    const start = membershipOffsets[i]!;
    const end = membershipOffsets[i + 1]!;
    const members = new Array<number>(end - start);
    for (let j = 0; j < members.length; j++) {
      members[j] = membershipValues[start + j]!;
    }
