...
export class ModelManager {
  ...
  private _uploadPrimitive(
    ...
    const primSuffix = primIndex > 0 ? `-p${primIndex}` : '';
    const label = `model-vertex-${modelId}${primSuffix}`;
    ...
    const indexLabel = `model-index-${modelId}${primSuffix}`;
    ...
    // Use paddedSize for getMappedRange — WebGPU requires size to be a
    // multiple of 4 when offset+size !== buffer.size. Odd uint16 index counts
    // (e.g., 56079 × 2 = 112158 bytes) would otherwise violate this rule.
    const mappedRange = indexBuffer.getMappedRange(0, Math.max(paddedSize, 4));
