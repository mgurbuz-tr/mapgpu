...
fn projectToClip(pos: vec3<f32>) -> vec4<f32> {
  ...
  // Phase 2 redesign: globeClippingZ override kaldırıldı — GPU perspective Z.
...
fn vs_main(input: VertexInput) -> VertexOutput {
  ...
  // Reverse-Z: + pushes closer to camera (near=1), clamp MIN to prevent hitting far (0).
  const LAYER_DEPTH_OFFSET: f32 = 0.0005;
  let adjustedZ = clipCurr.z + LAYER_DEPTH_OFFSET * clipCurr.w;
  let clampedZ = max(adjustedZ, clipCurr.w * 0.0001);
