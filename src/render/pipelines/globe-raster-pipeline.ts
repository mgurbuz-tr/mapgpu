...
struct TileUniforms {
  ...
  filters: vec4<f32>,
  // Imagery UV remap (for UV-clipped parent fallback): [offsetX, offsetY, scaleX, scaleY]
  // Exact tile: [0, 0, 1, 1] (identity). Parent fallback k-level up:
  //   offset = ((x % 2^k) / 2^k, (y % 2^k) / 2^k), scale = (1/2^k, 1/2^k).
  imageryUv: vec4<f32>,
};
...
// MapLibre custom Z: geometry-aware depth from clipping plane (reverse-Z).
// 3D mode reverse-Z: yakın point = yüksek NDC (~1-planeD), horizon = 0, arkası <0.
fn globeClippingZ(spherePos: vec3<f32>) -> f32 {
  return dot(spherePos, camera.clippingPlane.xyz) + camera.clippingPlane.w;
}
...
fn vs_main(input: VertexInput) -> VertexOutput {
  ...
  // Phase 2 redesign: globeClippingZ Z-override kaldırıldı. GPU perspective depth
  // kullanılır — reverse-Z sayesinde precision tüm frustum'da yeterli.
  ...
  // Reverse-Z: + puts closer (near=1), - puts farther (far=0).
  clipPos.z += LAYER_DEPTH_OFFSET * clipPos.w;
  clipPos.z -= tile.depthBias * clipPos.w;
  clipPos.z = max(clipPos.z, clipPos.w * 0.0001);
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
  ...
  // UV-clipped parent fallback: imageryUv = [offsetX, offsetY, scaleX, scaleY].
  // Exact tile → identity (0,0,1,1). Parent fallback → child's quadrant within parent.
  let imageryUv = input.texCoord * tile.imageryUv.zw + tile.imageryUv.xy;
  var color = textureSample(tileTexture, tileSampler, imageryUv);
...
export function createGlobeRasterPipeline(
  ...
  const pipeline = device.createRenderPipeline({
    ...
    depthStencil: {
      ...
      // Globe pipelines default to 'greater' for reverse-Z (3D mode, clearValue=0).
      // Caller her zaman ctx.depthConfig.compareFunc geçiyor ama default'un yanlış
      // olması ileride sessiz regresyon kaynağı olur (GLTF bug precedent).
...
function createSubdivisionMeshWithSkirts(
  ...
  const addSkirtStrip = (
    ...
    indices.push(b0, s0, b1, b1, s0, s1);
  };
