...
// Default colors
export const GRID_COLOR: [number, number, number, number]   = [0, 1, 1, 0.35];   // subtle cyan
export const DOT_COLOR: [number, number, number, number]    = [1, 1, 0.3, 0.92];    // bright yellow
export const BORDER_COLOR: [number, number, number, number] = [1, 0.4, 0, 0.85];    // orange
export const DOT_SIZE     = 3;  // pixels radius
export const BORDER_WIDTH = 2;  // pixels half-width
...
// Reverse-Z: yakın → 1, horizon → 0, arkası → negative (clipped).
...
fn projectUV(uv: vec2<f32>, height: f32) -> ProjResult {
  ...
  // Reverse-Z: + = yakın, max clamp prevents hitting far=0.
  cp.z += 0.0005 * cp.w;
  cp.z = max(cp.z, cp.w * 0.0001);
  var r: ProjResult;
  r.clipPos = cp;
  r.clipDot = cd;
  return r;
...
export function createTileDebugSuite(desc: TileDebugSuiteDescriptor): TileDebugSuite {
  ...
  const depthStencil = {
    ...
    depthCompare: globe ? (desc.depthCompare ?? 'less') : 'always',
  };
