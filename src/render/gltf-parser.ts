...
function parsePrimitiveMesh( // NOSONAR
  prim: GltfPrimitive,
  accessors: GltfAccessor[],
  bufferViews: GltfBufferView[],
  binChunk: Uint8Array,
...
function extractImageData(
  ...
  if (!textures?.[textureIndex]) return undefined;

  const tex = textures[textureIndex];
  if (tex.source === undefined) return undefined;

  const images = json.images;
  if (!images?.[tex.source]) return undefined;
...
function buildPrimFromMesh( // NOSONAR
  gltfMesh: GltfMesh,
  accessors: GltfAccessor[],
  bufferViews: GltfBufferView[],
  binChunk: Uint8Array,
...
function buildPrimitivesFromJson(json: GltfJson, binChunk: Uint8Array): ParsedGltf { // NOSONAR
