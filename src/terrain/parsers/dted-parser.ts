...
function parseUHL(buffer: ArrayBuffer): DTEDHeader {
  ...
  const sentinel = String.fromCodePoint(bytes[0] ?? 0, bytes[1] ?? 0, bytes[2] ?? 0);
  if (sentinel !== 'UHL') {
...
function parseDMSLon(bytes: Uint8Array, offset: number): number {
  ...
  const hem = String.fromCodePoint(bytes[offset + 7] ?? 69); // E
...
function parseDMSLat(bytes: Uint8Array, offset: number): number {
  ...
  const hem = String.fromCodePoint(bytes[offset + 6] ?? 78); // N
  const value = deg + min / 60 + sec / 3600;
...
function parseFixedInt(bytes: Uint8Array, offset: number, length: number): number {
  ...
  for (let i = 0; i < length; i++) {
    str += String.fromCodePoint(bytes[offset + i] ?? 32);
  }
  return Number.parseInt(str.trim(), 10) || 0;
}
...
export function extractCoordsFromFilename(fileName: string): [number, number] | null {
  ...
  const dirMatch = /([ew])(\d{3})\/([ns])(\d{2})/.exec(normalized);
  if (dirMatch) {
    let lon = Number.parseInt(dirMatch[2] ?? '0', 10);
    let lat = Number.parseInt(dirMatch[4] ?? '0', 10);
  ...
  const latFirstMatch = /([ns])(\d{2})([ew])(\d{3})/.exec(normalized);
  if (latFirstMatch) {
    let lat = Number.parseInt(latFirstMatch[2] ?? '0', 10);
    let lon = Number.parseInt(latFirstMatch[4] ?? '0', 10);
  ...
  const lonFirstMatch = /([ew])(\d{3})([ns])(\d{2})/.exec(normalized);
  if (lonFirstMatch) {
    let lon = Number.parseInt(lonFirstMatch[2] ?? '0', 10);
    let lat = Number.parseInt(lonFirstMatch[4] ?? '0', 10);
...
export function parseDTED(buffer: ArrayBuffer, options: ParseDTEDOptions = {}): DTEDTile { // NOSONAR
