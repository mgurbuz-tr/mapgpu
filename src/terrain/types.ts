import type { LayerBaseOptions } from '../layers/index.js';
import type { IWorker, TerrainLighting3DOptions, WorkerPoolRegistry } from '../core/index.js';
...
export interface DTEDLayerOptions extends LayerBaseOptions {
  ...
  /**
   * Optional worker pool registry (owned by ViewCore) used to offload DTED
   * parse off the main thread. When set, addLocalFile(s) routes the binary
   * blob through a Web Worker and only the parsed Int16Array elevations
   * cross the thread boundary. Falls back to main-thread parse on any error.
   */
  workerRegistry?: WorkerPoolRegistry;
  /**
   * Optional override for the terrain worker factory. Defaults to a module
   * worker loaded from the bundled `terrain.worker.js`. Intended for test
   * injection.
   */
  terrainWorkerFactory?: () => IWorker;
}
...
export interface TerrainRGBLayerOptions extends LayerBaseOptions {
  ...
  /**
   * Optional worker pool registry (owned by ViewCore). When set, hillshade
   * computation is offloaded to the terrain worker. Falls back to the
   * injected `_hillshadeFn` on missing registry or worker failure.
   */
  workerRegistry?: WorkerPoolRegistry;
  /**
   * Optional override for the terrain worker factory. Defaults to a module
