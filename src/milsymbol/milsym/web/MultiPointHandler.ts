...
export class MultiPointHandler {
    private static readonly _maxPixelWidth: number = 1920;
    private static readonly _minPixelWidth: number = 720;
     ...
     */
    public static NormalizeGECoordsToGEExtents(leftLongitude: number,
    ...
    static NormalizeCoordToGECoord(pt2d: Point2D): Point2D {
        let ptGeo: Point2D;
        try {
            let x: number = pt2d.getX();
            let y: number = pt2d.getY();
            while (x < -180) {
                x += 360;
            }
            while (x > 180) {
    ...
    static getGeoUL(geoCoords: Array<Point2D>): Point2D {
        ...
        try {
            let j: number = 0;
            let pt: Point2D;
            let left: number = geoCoords[0].getX();
            let top: number = geoCoords[0].getY();
            let right: number = geoCoords[0].getX();
            let bottom: number = geoCoords[0].getY();
            let n: number = geoCoords.length;
     ...
     * @return
     */
    static getReasonableScale(bbox: string, origScale: number): number {
        try {

            if(!rendererSettings.getAutoAdjustScale())
                return origScale;
            
            let bounds: string[] = bbox.split(",");
            let left: number = parseFloat(bounds[0]);
            let right: number = parseFloat(bounds[2]);
            let top: number = parseFloat(bounds[3]);
            let bottom: number = parseFloat(bounds[1]);

            let ul: POINT2 = new POINT2(left, top);
            let ur: POINT2 = new POINT2(right, top);

            let widthInMeters: number = 0;
            ...
            // Earth's circumference / 2
                ...
                widthInMeters = Geodesic.geodesic_distance(ul, ur).distance;
            }


            let minScale: number = widthInMeters / (MultiPointHandler._maxPixelWidth as number / rendererSettings.getDeviceDPI() / GeoPixelConversion.INCHES_PER_METER);
            if (origScale < minScale) {
                return minScale;
            }

            let maxScale: number = widthInMeters / (MultiPointHandler._minPixelWidth as number / rendererSettings.getDeviceDPI() / GeoPixelConversion.INCHES_PER_METER);
     ...
     * MilStdAttributes. pass in number[] for AM, AN and X; Strings for the
     * rest.
     * @param format
     * @return
     */
        ...
        format: number): string//,
    {
        ...
        let rect: Rectangle;
        let coordinates: string[] = controlPoints.split(" ");
        let tgl: TacticalGraphic = new TacticalGraphic();
        ...
        if (msLookup.getMSLInfo(symbolCode).getDrawRule() != DrawRules.AREA10) // AREA10 can support infinite points
            len = Math.min(len, msLookup.getMSLInfo(symbolCode).getMaxPointCount());
        ...
        if (bbox != null && bbox !== "") {
            ...
            if (bbox.includes(" "))//trapezoid
            {
                ...
                let ptGeo: Point2D;
                let n: number = bboxCoords.length;
            ...
            if (bboxCoords == null) {
                ...
                leftX = temp.getX() as number;
                topY = temp.getY() as number;

                pt2d = new Point2D(right, bottom);
                temp = ipc.GeoToPixels(pt2d);

                bottomY = temp.getY() as number;
                rightX = temp.getX() as number;
                ...
                width = Math.abs(rightX - leftX) as number;
                height = Math.abs(bottomY - topY) as number;
        ...
        tgl.symbolId = symbolCode;// "GFGPSLA---****X" AMBUSH symbol code
        tgl.Pixels = null;
        ...
        try {
            ...
            if (bboxCoords == null) {
                ...
                MultipointRenderer.renderWithPolylines(mSymbol, ipc, clipBounds);
            } else {
                MultipointRenderer.renderWithPolylines(mSymbol, ipc, bboxCoords);
            }
        ...
        if (debug === true) {
            ...
            if (tgl != null && tgl.Pixels != null)//pixels != null
            {
                console.log("Pixel: " + tgl.Pixels.toString());
            }
            if (bbox != null) {
    ...
    //ArrayList<ShapeInfo>shapes)
    {
        ...
        try {
            ...
            for (let label of modifiers) {
                ...
                let latitude: number = geoCoord.getY();
                let longitude: number = geoCoord.getX();
    ...
    static getOverscanClipBounds(rect: Rectangle, ipc: IPointConversion): Rectangle {
        ...
        let overScanScale = rendererSettings.getOverscanScale();
    ...
    //ArrayList<ShapeInfo>shapes)
    {
        ...
        try {
            ...
            MultipointRenderer.renderWithPolylines(mSymbol, ipc, rect);
            shapes = mSymbol.getSymbolShapes();
            modifiers = mSymbol.getModifierShapes();
    ...
    private static MilStdSymbolToSymbolInfo(symbol: MilStdSymbol): SymbolInfo {
        ...
        let modifierCount: number = modifiers.length;
        ...
        for (let j: number = 0; j < modifierCount; j++) {
            tiTemp = new TextInfo();
            siTemp = modifiers[j];
    ...
    static populateModifiers(saModifiers: Map<string, string>, saAttributes: Map<string, string>, symbol: MilStdSymbol): boolean {
        ...
        let patternFillType: number = 0;
        let hideOptionalLabels: boolean = false;
        let distanceUnit: DistanceUnit;
        let altitudeUnit: DistanceUnit;
        let pixelSize: number = 50;
        let keepUnitRatio: boolean = true;
        let patternScale: number = rendererSettings.getPatternScale();
        ...
        try {
            ...
            if (saModifiers != null) {
                ...
                if (saModifiers.has(Modifiers.AS_COUNTRY)) {
                    ...
                    if (SymbolID.getCountryCode(symbol.getSymbolID()) > 0 && gencLookup.get3CharCode(SymbolID.getCountryCode(symbol.getSymbolID())) !== "") {
                        modifiers.set(Modifiers.AS_COUNTRY, gencLookup.get3CharCode(SymbolID.getCountryCode(symbol.getSymbolID())));
                    }
                }
            ...
            if (SymbolUtilities.getBasicSymbolID(symbol.getSymbolID()) === "25242200") {
                if (symbol.getModifiers_AM_AN_X(Modifiers.AN_AZIMUTH) != null
                    ...
                    let anCount: number = symbol.getModifiers_AM_AN_X(Modifiers.AN_AZIMUTH).length;
                    let amCount: number = symbol.getModifiers_AM_AN_X(Modifiers.AM_DISTANCE).length;
                    let am: Array<number>;
    ...
    private static KMLize(id: string, 
        ...
        let len2: number = modifiers.length;
    ...
    static getIdealTextBackgroundColor(fgColor: Color): Color {
        ...
        try {
            ...
            let hsbvals: number[] = new Array<number>(3);
            ...
            if (fgColor != null) {/*
                ...
                let nThreshold: number = rendererSettings.getTextBackgroundAutoColorThreshold();//160;
                let bgDelta: number = Math.trunc((fgColor.getRed() * 0.299) + (fgColor.getGreen() * 0.587) + (fgColor.getBlue() * 0.114));
    ...
    private static LabelToGeoJSONString(shapeInfo: ShapeInfo, ipc: IPointConversion, normalize: boolean, textColor: Color, textBackgroundColor: Color): string {
        ...
        let latitude: number = Math.round(geoCoord.getY() * 100000000.0) / 100000000.0;
        let longitude: number = Math.round(geoCoord.getX() * 100000000.0) / 100000000.0;
        let angle: number = shapeInfo.getModifierAngle();
        ...
        let justify: number = shapeInfo.getTextJustify();
    ...
    private static ShapeToGeoJSONString(shapeInfo: ShapeInfo, ipc: IPointConversion, normalize: boolean): string {
        ...
        let lineCap: number = stroke.getEndCap();
        ...
        if (stroke.getDashArray() != null) {
            ...
            sda = da[0].toString();
            if (da.length > 1) {
                for (let i: number = 1; i < da.length; i++) {
        ...
        for (let i: number = 0; i < shapesArray.length; i++) {
            ...
            for (let j: number = 0; j < pointList.length; j++) {
    ...
    private static ShapeToKMLString(shapeInfo: ShapeInfo,
        ...
        if (lineColor != null) {
            ...
            if (stroke != null) {
                lineWidth = stroke.getLineWidth() as number;
            }
        ...
        let shapesArray: Point2D[][] = shapeInfo.getPolylines();
        let len: number = shapesArray.length;
        kml += ("<MultiGeometry>");
    ...
    private static ShapeToJSONString(shapeInfo: ShapeInfo, ipc: IPointConversion, geMap: boolean, normalize: boolean): string {
        ...
        let shapesArray: Point2D[][] = shapeInfo.getPolylines();
        let n: number = shapesArray.length;
    ...
    private static LabelToKMLString(shapeInfo: ShapeInfo, ipc: IPointConversion, normalize: boolean, textColor: Color): string {
        ...
        let kmlScale: number = rendererSettings.getKMLLabelScale();
    ...
    public static canRenderMultiPoint(symbolID: string, modifiers: Map<string, string>, numPoints: number): string {
        try {
            let basicID: string = SymbolUtilities.getBasicSymbolID(symbolID);
            let info: MSInfo = msLookup.getMSLInfo(symbolID);
            ...
            let drawRule: number = info.getDrawRule();
    ...
    private static hasRequiredModifiers(symbolID: string, drawRule: number, AM: Array<number>, AN: Array<number>): string {
        ...
        symbolAttributes: Map<string, string>): MilStdSymbol
    {
        ...
        try {
            ...
            let tg: TacticalGraphic = MultipointRenderer.createTacticalGraphicFromMilStdSymbolBasicShape(mSymbol, ipc, basicShapeType);
            ...
            if (MultipointRenderer.intersectsClipArea(tg, ipc, clipArea)) {
                MultipointRenderer.render_GE(tg, shapeInfos, modifierShapeInfos, ipc, clipArea);
            }
            mSymbol.setSymbolShapes(shapeInfos);
            mSymbol.setModifierShapes(modifierShapeInfos);
            mSymbol.setWasClipped(tg.wasClipped);
            shapes = mSymbol.getSymbolShapes();
            modifiers = mSymbol.getModifierShapes();
