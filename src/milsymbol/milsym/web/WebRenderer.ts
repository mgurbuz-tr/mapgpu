...
export class WebRenderer /* extends Applet */ {
    ...
    public static readonly MIN_ALT_DEFAULT: number = 0.0;
    public static readonly MAX_ALT_DEFAULT: number = 100.0;
    public static readonly RADIUS1_DEFAULT: number = 50.0;
    public static readonly RADIUS2_DEFAULT: number = 100.0;
    public static readonly LEFT_AZIMUTH_DEFAULT: number = 0.0;
    public static readonly RIGHT_AZIMUTH_DEFAULT: number = 90.0;
    ...
    private static init(): void {
        try {
            if (WebRenderer._initSuccess === false) {
                ...
                rendererSettings.setTextBackgroundMethod(
                    RendererSettings.TextBackgroundMethod_OUTLINE);
                //rendererSettings.setLabelForegroundColor(Color.BLACK);
                //rendererSettings.setLabelBackgroundColor(new Color(255, 255, 255, 200));
                rendererSettings.setLabelFont("arial", Font.PLAIN, 12);
 ...
 /**\
     ...
     */
    public static setLoggingLevel(level: LogLevel | number): void {
        if (level instanceof LogLevel) {
            ...
            try {
                ...
                ErrorLogger.LogMessage("WebRenderer", "setLoggingLevel(number)",
                    "Logging level set to: " + ErrorLogger.getLevel().getName(),
                    LogLevel.CONFIG);
            } catch (exc) {
                if (exc instanceof Error) {
                    ErrorLogger.LogException("WebRenderer", "setLoggingLevel(number)", exc, LogLevel.INFO);
                } else {
                    throw exc;
                }
            }
     ...
     * @param size 
     */
    public static setTacticalGraphicPointSize(size: number): void {
     ...
     * @param size 
     */
    public static setUnitPointSize(size: number): void {
    ...
    /*    public static void setModifierTextColor(String hexColor)
        {
            ...
            rendererSettings.setLabelForegroundColor(textColor);
        }*/
    ...
    public static RenderSymbol(id: string, name: string, description: string,
        ...
        scale: number, bbox: string, modifiers: Map<string, string>, attributes: Map<string, string>, format: number): string {
        let output: string = "";
        try {
    ...
    public static RenderSymbol2D(id: string, name: string, description: string, symbolCode: string, controlPoints: string,
        pixelWidth: number, pixelHeight: number, bbox: string, modifiers: Map<string, string>,
        attributes: Map<string, string>, format: number): string {
        let output: string = "";
        try {
    ...
    public static RenderMultiPointAsMilStdSymbol(id: string, name: string, description: string, symbolCode: string,
        controlPoints: string, altitudeMode: string, scale: number, bbox: string, modifiers: Map<string, string>, attributes: Map<string, string>): MilStdSymbol {
        ...
        try {
            ...
            //            {
            //                int size = rendererSettings.getDefaultPixelSize();
            //
     ...
     * @return MilStdSymbol
     */
    public static RenderBasicShapeAsMilStdSymbol(id: string, name: string, description: string, basicShapeType: number,
        controlPoints: string, altitudeMode: string, scale: number, bbox: string, modifiers: Map<string, string>, attributes: Map<string, string>): MilStdSymbol {
        let mSymbol: MilStdSymbol;
        try {
    ...
    public static RenderBasicShape(id: string, name: string, description: string,
        basicShapeType: number, controlPoints: string, altitudeMode: string,
        scale: number, bbox: string, modifiers: Map<string, string>, attributes: Map<string, string>, format: number): string {
        let output: string = "";
        try {
            if (SymbolUtilities.isBasicShape(basicShapeType))
    ...
    */
    ...
    public static getRectangleCorners(endpoints: string, width: number) {
        ...
        const { a12: attitudeVal } = Geodesic.geodesic_distance(pt0, pt1);

        points.push(Geodesic.geodesic_coordinate(pt0, width / 2.0, attitudeVal - 90)); //top left
        points.push(Geodesic.geodesic_coordinate(pt0, width / 2.0, attitudeVal + 90)); //top right
        points.push(Geodesic.geodesic_coordinate(pt1, width / 2.0, attitudeVal + 90));  //bottom right
        points.push(Geodesic.geodesic_coordinate(pt1, width / 2.0, attitudeVal - 90)); //bottom left
