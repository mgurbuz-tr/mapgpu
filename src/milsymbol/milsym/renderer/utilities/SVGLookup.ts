...
import { RendererSettings, rendererSettings } from "./RendererSettings"
...
export class SVGLookup
{
    ...
    /**
     * Initialize SVGLookup in lazy mode — no monolithic JSON is loaded upfront.
     * Only common SVG data (frames, amplifiers) is loaded immediately.
     * Symbol-set-specific data is loaded on demand via ensureSymbolSet().
     *
     * @returns Promise that resolves when common data is ready
     */
    public static async initLazy(): Promise<SVGLookup> {
        SVGLookup._lazyMode = true;

        if (!SVGLookup._instance) {
            // Create instance without eager loading (init() will skip monolithic load)
            SVGLookup._instance = new SVGLookup();
        }

        // Load common SVG data (frames, amplifiers, echelons) for both versions
        const loader = dataLoader;
        const [commonD, commonE] = await Promise.all([
            loader.loadSymbolSet('common', '2525d'),
            loader.loadSymbolSet('common', '2525e')
        ]);

        // Populate maps with common data
        if (commonD) SVGLookup._instance.populateLookup(commonD, SymbolID.Version_2525Dch1);
        if (commonE) SVGLookup._instance.populateLookup(commonE, SymbolID.Version_2525E);

        SVGLookup._loadedSymbolSetsD.add('common');
        SVGLookup._loadedSymbolSetsE.add('common');
    ...
    /**
     * Ensure a symbol set's SVG data is loaded and available for synchronous lookup.
    ...
    public static async ensureSymbolSet(symbolSet: string, version: number): Promise<void> {
        if (!SVGLookup._lazyMode) return; // Everything already loaded

        const ss = symbolSet.padStart(2, '0');
        const isE = version >= SymbolID.Version_2525E;
        const loadedSet = isE ? SVGLookup._loadedSymbolSetsE : SVGLookup._loadedSymbolSetsD;
        const versionStr = isE ? '2525e' : '2525d';

        if (loadedSet.has(ss)) return; // Already loaded

        // Dedup concurrent loads for the same SS
        const loadKey = `${versionStr}:${ss}`;
        if (SVGLookup._loadingPromises.has(loadKey)) {
            return SVGLookup._loadingPromises.get(loadKey);
        }

        const loadPromise = (async () => {
            try {
                const loader = dataLoader;
                const data = await loader.loadSymbolSet(ss, versionStr);
                if (data) {
                    SVGLookup._instance.populateLookup(data, version);
                    loadedSet.add(ss);
                }
            } finally {
                SVGLookup._loadingPromises.delete(loadKey);
            }
        })();

        SVGLookup._loadingPromises.set(loadKey, loadPromise);
        return loadPromise;
    }

    /**
     * Ensure all symbol sets needed for a full SIDC are loaded.
     * Extracts the symbol set from the SIDC and loads it.
     */
    public static async ensureForSIDC(symbolID: string): Promise<void> {
        if (!SVGLookup._lazyMode) return;

        const ss = symbolID.substring(4, 6);
        const version = SymbolID.getVersion(symbolID);

        await SVGLookup.ensureSymbolSet(ss, version);
    }
    ...
    private init(): void {
        ...
        if (!SVGLookup._lazyMode && typeof jsond === 'object')
        {
    ...
    {
        if (!SVGLookup._lazyMode) return true;
        const ss = symbolSet.padStart(2, '0');
        if (version >= SymbolID.Version_2525E) {
            return SVGLookup._loadedSymbolSetsE.has(ss);
        }
        return SVGLookup._loadedSymbolSetsD.has(ss);
    }
    ...
    // ─── Synchronous Lookup (original API) ───────────────────────────────────
    ...
    /**
     * Async version of getSVGLInfo — ensures the required symbol set is loaded
     * before performing the lookup. Safe to use in both eager and lazy modes.
     *
     * @param id SVG element ID
     * @param version SymbolID version constant
     * @param symbolSet Two-digit symbol set code (if known, avoids re-parsing)
     */
    public async getSVGLInfoAsync(id: string, version: number, symbolSet?: string): Promise<SVGInfo | null> {
        if (SVGLookup._lazyMode && symbolSet) {
            await SVGLookup.ensureSymbolSet(symbolSet, version);
        }
        return this.getSVGLInfo(id, version);
    }
    ...
    public static getMainIconID(symbolID: string): string {
        ...
        if (ss === SymbolID.SymbolSet_MineWarfare) {
            if (rendererSettings.getSeaMineRenderMethod() === RendererSettings.SeaMineRenderMethod_ALT ||
    ...
    public static resetForTesting(): void {
        SVGLookup._instance = null;
        SVGLookup._initCalled = false;
        SVGLookup._isReady = false;
        SVGLookup._lazyMode = false;
        SVGLookup._loadedSymbolSetsD = new Set();
        SVGLookup._loadedSymbolSetsE = new Set();
        SVGLookup._loadingPromises = new Map();
    }
...
export const svgLookup: SVGLookup = new Proxy({} as SVGLookup, {
    get(_target, prop) {
