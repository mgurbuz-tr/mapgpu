...
import { IPointConversion } from "./IPointConversion"
...
export class PointConversion implements IPointConversion {
    protected _pixelWidth: number = 0;
    protected _PixelHeight: number = 0;
    protected _geoTop: number = 0;
    protected _geoLeft: number = 0;
    protected _geoBottom: number = 0;
    protected _geoRight: number = 0;
    protected _normalize: boolean = true;
    //pixels to geo
    //double _geoMultiplierX = 0;
    //double _geoMultiplierY = 0;
    //geo to pixels
    protected _pixelMultiplierX: number = 0;
    protected _pixelMultiplierY: number = 0;
    public set_normalize(value: boolean): void {
        this._normalize = value;
    }

    public constructor(pixelWidth: number, pixelHeight: number,
        geoTop: number, geoLeft: number,
        geoBottom: number, geoRight: number) {/*
    ...
    public UpdateExtents(pixelWidth: number, pixelHeight: number,
        geoTop: number, geoLeft: number,
        geoBottom: number, geoRight: number): void {
        ...
        this._pixelMultiplierX = (this._geoRight - this._geoLeft) / (this._pixelWidth as number);
        this._pixelMultiplierY = (this._geoTop - this._geoBottom) / (this._PixelHeight as number);

        //diagnostic 12-18-12
        if (this._geoRight - this._geoLeft < -180) {
            this._pixelMultiplierX = (this._geoRight - this._geoLeft + 360) / (this._pixelWidth as number);
        }
        if (this._geoRight - this._geoLeft > 180) {
            this._pixelMultiplierX = (360 - (this._geoRight - this._geoLeft)) / (this._pixelWidth as number);
        }
        //end section
    ...
    public GeoToPixels(...args: unknown[]): Point | Point2D {
        ...
        let calcValue: number = coord.getX() - this._geoLeft;
        if (this._normalize) {
            if (calcValue < -180) {
    ...
    public getUpperLat(): number {
        return this._geoTop;
    }

    public getLowerLat(): number {
        return this._geoBottom;
    }

    public getLeftLon(): number {
        return this._geoLeft;
    }

    public getRightLon(): number {
