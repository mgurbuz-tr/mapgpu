...
import { GeoPixelConversion3D } from "./GeoPixelConversion3D"
...
export class PointConverter3D implements IPointConversion {
    ...
    public PixelsToGeo(...args: unknown[]): Point2D | Point2D {
        if (args[0] instanceof Point) {
            ...
            let pt2dGeo: Point2D;
            try {
                let y: number = GeoPixelConversion3D.y2lat(pixel.getY(), this._scale, this._controlLat, this._metersPerPixel);
                let x: number = GeoPixelConversion3D.x2long(pixel.getX(), this._scale, this._controlLong, y, this._metersPerPixel);
    ...
    public GeoToPixels(...args: unknown[]): Point | Point2D {
        ...
        if (args[0] instanceof Point2D) {
            ...
            let ptPixels: Point;
            try {
                let y: number = GeoPixelConversion3D.lat2y(coord.getY(), this._scale, this._controlLat, this._metersPerPixel);
                let x: number = GeoPixelConversion3D.long2x(coord.getX(), this._scale, this._controlLong, coord.getY(), this._metersPerPixel);
                ptPixels = new Point();
