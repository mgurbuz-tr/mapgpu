...
import { BufferedImage } from "../../graphics/BufferedImage"
...
import { SymbolDimensionInfo } from "./SymbolDimensionInfo"
...
export class ImageInfo implements SymbolDimensionInfo {
	...
	private _X: number = 0;
	private _Y: number = 0;
	private _symbolCenterX: number = 0;
	private _symbolCenterY: number = 0;
	 ...
	 */
	public constructor(image: ImageBitmap, x: number, y: number);
	 ...
	 * @deprecated
	 */
	public constructor(image: ImageBitmap, x: number, y: number, symbolCenterX: number, symbolCenterY: number);
	 ...
	 */
	public constructor(image: ImageBitmap, x: number, y: number, symbolCenterX: number, symbolCenterY: number, symbolBounds: Rectangle2D);
	public constructor(...args: unknown[]) {
		switch (args.length) {
			...
			case 3: 
			{
				...
				{
					const [image, x, y] = args as [ImageBitmap, number, number];
					this._Image = image;
					this._X = x;
					this._Y = y;
				...
				else if(args[1] instanceof Point2D)
				{
					...
					this._Image = bi;
					this._symbolCenterX = centerPoint.getX() as number;
					this._symbolCenterY = centerPoint.getY() as number;
			...
			case 5: {
				const [image, x, y, symbolCenterX, symbolCenterY] = args as [ImageBitmap, number, number, number, number];
			...
			case 6: {
				const [image, x, y, symbolCenterX, symbolCenterY, symbolBounds] = args as [ImageBitmap, number, number, number, number, Rectangle2D];
	 ...
	 * @return {@link Integer}
	 */
	public getSymbolCenterX(): number {
	 ...
	 * @return {@link Integer}
	 */
	public getSymbolCenterY(): number {
	...
	public static CenterImageOnPoint(image: ImageBitmap, point: Point2D): ImageBitmap {
		...
		try {
			if (point.getY() > height - point.getY()) {
				height = (point.getY() * 2.0) as number;
				y = 0;
			}
			else {
				height = ((height - point.getY()) * 2) as number;
				y = ((height / 2) - point.getY()) as number;
			}

			if (point.getX() > width - point.getX()) {
				width = (point.getX() * 2.0) as number;
				x = 0;
			}
			else {
				width = ((width - point.getX()) * 2) as number;
				x = ((width / 2) - point.getX()) as number;
			}
