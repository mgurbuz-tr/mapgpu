...
import { msLookup } from "./MSLookup"
import { RendererSettings, rendererSettings } from "./RendererSettings"
import { SVGLookup, svgLookup } from "./SVGLookup"
...
export class SymbolUtilities {
    ...
    public static reconcileSymbolID(symbolID: string): string {
        ...
        try {
            ...
            let v: number = SymbolID.getVersion(symbolID);
            ...
            let a: number = SymbolID.getAffiliation(symbolID);
            if (a > 6) {
            ...
            let s: number = SymbolID.getStatus(symbolID);
            ...
            //root symbol for symbol set
                ...
                if (svgLookup.getSVGLInfo(SVGLookup.getMainIconID(newID + ec + "0000"), v) == null) {
    ...
    public static getLineColorOfAffiliation(symbolID: string): Color | null {
        ...
        let set: number = SymbolID.getSymbolSet(symbolID);
        let affiliation: number = SymbolID.getAffiliation(symbolID);
        let symStd: number = SymbolID.getVersion(symbolID);
        ...
        try {
            ...
            if (symbolSet === SymbolID.SymbolSet_ControlMeasure) {
                let entity: number = SymbolID.getEntity(symbolID);
                let entityType: number = SymbolID.getEntityType(symbolID);
                let entitySubtype: number = SymbolID.getEntitySubtype(symbolID);
                ...
                {
                    // If not black then color will be set in METOC.SetMeTOCProperties()
                    retColor = Color.BLACK;
                    ;
                }
                else {
                    if (set === SymbolID.SymbolSet_MineWarfare && (rendererSettings.getSeaMineRenderMethod() === RendererSettings.SeaMineRenderMethod_MEDAL)) {
    ...
    public static hasDefaultFill(strSymbolID: string): boolean {
        let ec: number = SymbolID.getEntityCode(strSymbolID);
        switch (ec) {
            case 200101:
            case 200201:
            case 200202:
    ...
    public static isTacticalGraphic(strSymbolID: string): boolean {
        try {
            let ss: number = SymbolID.getSymbolSet(strSymbolID);
    ...
    public static isMultiPoint(symbolID: string): boolean {
        ...
        let drawRule: number = msi.getDrawRule();
        let ss: number = msi.getSymbolSet();
    ...
    public static isTGSPWithSpecialModifierLayout(strSymbolID: string): boolean {
        try {
            let ss: number = SymbolID.getSymbolSet(strSymbolID);
            let entityCode: number = SymbolID.getEntityCode(strSymbolID);
    ...
    public static getFillColorOfAffiliation(symbolID: string): Color | null {
        ...
        try {
            ...
            if (SymbolID.getSymbolSet(symbolID) === SymbolID.SymbolSet_ControlMeasure) {
                switch (entityCode) {
                    case 200101: {
                        retColor = new Color(255, 155, 0, (.25 * 255) as number);
                        break;
                    }
                    ...
                    case 200202:
                    case 200600: {
                        retColor = new Color(85, 119, 136, (.25 * 255) as number);
                        break;
                    }

                    case 200700: {
                        retColor = new Color(51, 136, 136, (.25 * 255) as number);
                        break;
                    }
                ...
                if (SymbolID.getSymbolSet(symbolID) === SymbolID.SymbolSet_MineWarfare &&
                    (rendererSettings.getSeaMineRenderMethod() === RendererSettings.SeaMineRenderMethod_MEDAL) &&
     ...
     * @return 
     */
    public static isGreenProtectionGraphic(entity: number, entityType: number, entitySubtype: number): boolean;
    public static isGreenProtectionGraphic(...args: unknown[]): boolean {
        switch (args.length) {
            ...
            case 3: {
                const [entity, entityType, entitySubtype] = args as [number, number, number];
    ...
    public static isHQ(symbolID: string): boolean {
        let hq: number = SymbolID.getHQTFD(symbolID);
    ...
    public static getCMSymbolAnchorPoint(symbolID: string, bounds: Rectangle2D): Point {
        ...
        let centerX: number = bounds.getWidth() / 2;
        let centerY: number = bounds.getHeight() / 2;
        ...
        //and meteorological
        if (ss === SymbolID.SymbolSet_ControlMeasure) {
            drawRule = msLookup.getMSLInfo(symbolID).getDrawRule();
     ...
     */
    public static getEchelonText(echelon: number): string {
        let text: string;
     ...
     * @return 
     */
    public static getUnitRatioHeight(symbolID: string): number {
     ...
     * @return 
     */
    public static getUnitRatioWidth(symbolID: string): number {
        let ver: number = SymbolID.getVersion(symbolID);
        let aff: number = SymbolID.getAffiliation(symbolID);
     ...
     */
    public static isBasicShape(linetype: number): boolean {
        switch (linetype) {
