...
import { DrawRules } from "./utilities/DrawRules"
...
export class SinglePointSVGRenderer {
        ...
        try {
            ...
            if (this._buffer == null || this._buffer === undefined) 
            {
                if(SinglePointSVGRenderer.OSCDefined)//(SinglePointSVGRenderer.OSCDefined)
                    ...
                    this._buffer = createMeasureCanvas(8,8);
    ...
    public RenderUnit(symbolID: string, modifiers: Map<string, string>, attributes: Map<string, string>): SVGSymbolInfo {
        ...
        try {
            ...
            if(SymbolID.getSymbolSet(symbolID)==SymbolID.SymbolSet_MineWarfare && rendererSettings.getSeaMineRenderMethod()==RendererSettings.SeaMineRenderMethod_MEDAL)
            {
            ...
            if (ii == null) //ii is always null because the above check is commented out
            {
                ...
                siFrame = svgLookup.getSVGLInfo(frameID, version);
                siIcon = svgLookup.getSVGLInfo(iconID, version);
                ...
                if (siIcon == null) {
                    if (iconID.substring(2, 8) === "000000" === false && msLookup.getMSLInfo(symbolID) == null) {

                        siIcon = svgLookup.getSVGLInfo("98100000", version);
                    }
                    //inverted question mark
                    else {
                        if (SymbolID.getSymbolSet(symbolID) === SymbolID.SymbolSet_Unknown) {

                            siIcon = svgLookup.getSVGLInfo("00000000", version);
                        }
                ...
                if(rendererSettings.getScaleMainIcon())
                    siIcon = RendererUtilities.scaleIcon(symbolID,siIcon);

                siMod1 = svgLookup.getSVGLInfo(mod1ID, version);
                siMod2 = svgLookup.getSVGLInfo(mod2ID, version);
                ...
                // Apply dashed frame for Exercise/Simulation context
                let context: number = SymbolID.getContext(symbolID);
                if (context === SymbolID.StandardIdentity_Context_Exercise ||
                    context === SymbolID.StandardIdentity_Context_Simulation) {
                    strSVGFrame = strSVGFrame.replace("<rect ", "<rect stroke-dasharray=\"12 8\" ");
                    strSVGFrame = strSVGFrame.replace("<polygon ", "<polygon stroke-dasharray=\"12 8\" ");
                    strSVGFrame = strSVGFrame.replace("<circle ", "<circle stroke-dasharray=\"12 8\" ");
                    strSVGFrame = strSVGFrame.replace("<ellipse ", "<ellipse stroke-dasharray=\"12 8\" ");
                }
                ...
                if (pixelSize > 0 && keepUnitRatio === true) {
                    ...
                    if (heightRatio > widthRatio) {
                        pixelSize = ((pixelSize / 1.5) * heightRatio) as number;
                    }
                    else {
                        pixelSize = ((pixelSize / 1.5) * widthRatio) as number;
                    }
                }
                if (pixelSize > 0) {
                    let p: number = pixelSize;
                    let h: number = rect.getHeight() as number;
                    let w: number = rect.getWidth() as number;
                ...
                centerOctagon = new Point((centerOctagon.x * ratio) as number, (centerOctagon.y * ratio) as number);
                ...
                //symbolBounds.top = 0;
                RectUtilities.shift(symbolBounds, 0, -symbolBounds.getY() as number);
            ...
            let svgWidth: number = (si.getImageBounds().getWidth() + widthOffset) as number;
            let svgHeight: number = si.getImageBounds().getHeight() as number;
            ...
            let transX: number = si.getImageBounds().getX() * -1;
            let transY: number = si.getImageBounds().getY() * -1;
    ...
    public RenderSP(symbolID: string, modifiers: Map<string, string>, attributes: Map<string, string>): SVGSymbolInfo | null {
        ...
        let ii: ImageInfo;
        let drawRule: number = 0;

        try {
            msi = msLookup.getMSLInfo(symbolID);
            ...
            if(rendererSettings.getActionPointDefaultFill()) 
            {
            ...
            if (attributes != null) {
                ...
                if (!(asIcon === true || hasAPFill === true))//don't outline icons because they're not going on the map
                {
                    if (attributes.has(MilStdAttributes.OutlineSymbol)) {
                        ...
                        outlineSymbol = rendererSettings.getOutlineSPControlMeasures();
                    }
            ...
            if (si == null)//*/
            {
                ...
                if (pixelSize > 0) {
                    ...
                    if (borderPadding > (h * 0.1)) {
                        borderPadding = (h * 0.1) as number;
                    }
                    else {
                        if (borderPadding > (w * 0.1)) {
                            borderPadding = (w * 0.1) as number;
                        }
                    }
                    //*/
                ...
                if (msi.getSymbolSet() === SymbolID.SymbolSet_ControlMeasure && outlineSymbol && borderPadding !== 0) {
                    ...
                    offset = borderPadding as number / 2;//4;
        ...
        try
        {
            ...
            if(drawAsIcon===false)//don't outline icons because they're not going on the map
            {
                if(attributes.has(MilStdAttributes.OutlineSymbol))
                    ...
                    drawCustomOutline = rendererSettings.getOutlineSPControlMeasures();
            }
        }
        catch (e)
        {
    ...
    private getSVGString(symbolID: string, isOutline: boolean): string {
        let version: number = SymbolID.getVersion(symbolID);
        let svgi: SVGInfo = svgLookup.getSVGLInfo(SymbolUtilities.getBasicSymbolID(symbolID), version);
...
export const singlePointSVGRenderer = SinglePointSVGRenderer.getInstance();
