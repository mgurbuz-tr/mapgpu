...
import { CChannelPoints2 } from "./channel-points"
...
export class Channels {
    private static readonly maxLength: number = 100;//max arrow size
    private static readonly minLength: number = 5;	//max arrow size
    ...
    private static ConnectArrayTrueDouble(nWidth: number,
        nCounter: number,
        pLinePoints: POINT2[],
        pResultChannelPoints: CChannelPoints2[]): CChannelPoints2[] {
        try {
            //declarations
            let nPointCounter: number = 0;
            let nDiff1X: number = 0;
            let
                nDiff2X: number = 0;
            let
                nDiff1Y: number = 0;
    ...
    private static GetChannel2Double(nChannelWidth: number,
        vblCounter: number,
        pLinePoints: POINT2[],
        pResultChannelPoints: CChannelPoints2[]): CChannelPoints2[] {
        try {
    ...
    private static FenceType(linetype: number): number {
        let bolResult: number = 0;
        try {
            switch (linetype) {
    ...
    */
    ...
    protected static CoordIL2Double(nPrinter: number,
        pLinePoints: POINT2[],
        nUpperLower: number,
        vblCounter: number,
        linetype: number,
        vblChannelWidth: number): POINT2[] {
        let pLinePoints2: POINT2[] = new Array<POINT2>(vblCounter);
        try {
            //declarations
            let j: number = 0;
            let channelWidth: number = 20;
     ...
     * @param dOffsetFactor
     */
    private static GetAXADDouble(dPrinter: number,
        pLowerLinePoints: POINT2[],
        lLowerCounter: number,
        pUpperLinePoints: POINT2[],
        lUpperCounter: number,
        ArrowLinePoint: POINT2,
        pLinePoints: POINT2[],
        vbiDrawThis: number,
        dOffsetFactor: number): void {
        try {
            let j: number = 0;
            let
                lCounter: number = lLowerCounter + lUpperCounter + 8;
            let x: number = 0;
            let y: number = 0;
            ...
            //LineUtility.adjustCATKBYFIREControlPoint as called by ChannelUtils.DrawChannel
            ...
            let dist: number = LineUtility.calcDistance(pLowerLinePoints[lLowerCounter - 1], pLowerLinePoints[lLowerCounter - 2]);
            if (vbiDrawThis === TacticalLines.CATKBYFIRE) {
                if (dist > 45) {
                    let midPt: POINT2 = LineUtility.midPoint(pLowerLinePoints[lLowerCounter - 2], pUpperLinePoints[lUpperCounter - 2], 0);
                    ArrowLinePoint = LineUtility.extendAlongLine(ArrowLinePoint, midPt, 45);
                    pLowerLinePoints[lLowerCounter - 1] = LineUtility.extendAlongLine(pLowerLinePoints[lLowerCounter - 1], pLowerLinePoints[lLowerCounter - 2], 45);//will be 45 if Oculus adjusts control point
                    pUpperLinePoints[lUpperCounter - 1] = LineUtility.extendAlongLine(pUpperLinePoints[lUpperCounter - 1], pUpperLinePoints[lUpperCounter - 2], 45);//will be 45 if Oculus adjusts control point
                }
            }
            //end section
            ...
            EndLinePoint.x = Math.trunc((pLowerLinePoints[lLowerCounter - 1].x +
                pUpperLinePoints[lUpperCounter - 1].x) as number / 2);

            EndLinePoint.y = Math.trunc((pLowerLinePoints[lLowerCounter - 1].y +
                pUpperLinePoints[lUpperCounter - 1].y) as number / 2);


            x = (pLowerLinePoints[lLowerCounter - 1].x - pUpperLinePoints[lUpperCounter - 1].x) as number;
            y = (pLowerLinePoints[lLowerCounter - 1].y - pUpperLinePoints[lUpperCounter - 1].y) as number;
            x = x * x;
            y = y * y;
     ...
     */
    static GetChannelArray2Double(nPrinter: number,
        pLinePoints: POINT2[],
        nUpperLower: number,
        vblCounter: number,
        vbiDrawThis: number,
    ...
    private static GetTrueEndPointDouble(nWidth: number,
        EndLinePoint: POINT2,
        NextLinePoint: POINT2,
        lLast: number): CChannelPoints2 {
        ...
        try {
            ...
            let
                b: number = 0;
            let
                bPerpendicular: number = 0;
            let
                Upperb: number = 0;
            let
                Lowerb: number = 0;
            let
                dWidth: number = nWidth as number;
            let bolVertical: number = 0;
            //end declarations

            const linesResult0 = LineUtility.calcTrueLines(nWidth, EndLinePoint, NextLinePoint);
            bolVertical = linesResult0.result;
            m = linesResult0.values[0];
            b = linesResult0.values[1];
            Upperb = linesResult0.values[3];
            Lowerb = linesResult0.values[5];
            ...
            if (bolVertical !== 0 && m !== 0)	//lines are neither vertical nor horizontal
            {
                ...
                LinePoint1 = LineUtility.calcTrueIntersect(m, Upperb, -1 / m, bPerpendicular, 1, 1, 0, 0);
                LinePoint2 = LineUtility.calcTrueIntersect(m, Lowerb, -1 / m, bPerpendicular, 1, 1, 0, 0);
    ...
    private static ConnectTrueDouble2(nWidth: number,
        LinePoint1: POINT2,
        LinePoint2: POINT2,
        LinePoint3: POINT2,
        lOrient: number): CChannelPoints2 {
        ...
        try {
            ...
            let bolVerticalSlope1: number = 0;
            let bolVerticalSlope2: number = 0;
            //end declarations

            //Call CalcLines function for first two points (LinePoint1, LinePoint2)
            //and put parameters into the proper variables
            const linesResult1 = LineUtility.calcTrueLines(nWidth, LinePoint1, LinePoint2);
            bolVerticalSlope1 = linesResult1.result;
            if (bolVerticalSlope1 !== 0)	//line is not vertical
            {
                m1 = linesResult1.values[0];
                b1 = linesResult1.values[1];
                Upperb1 = linesResult1.values[5];
                Lowerb1 = linesResult1.values[3];
            }

            //Call CalcLines function for next two points (LinePoint2, LinePoint3)
            const linesResult2 = LineUtility.calcTrueLines(nWidth, LinePoint2, LinePoint3);
            bolVerticalSlope2 = linesResult2.result;
            if (bolVerticalSlope2 !== 0)	//line is not vertical
            {
                m2 = linesResult2.values[0];
                b2 = linesResult2.values[1];
                Upperb2 = linesResult2.values[5];
                Lowerb2 = linesResult2.values[3];
            }
            ...
            switch (lOrient) {
                case 0: {
                    ...
                    const int0a = LineUtility.calcTrueIntersectAbs(m1, Upperb1, m2, Upperb2, LinePoint2, bolVerticalSlope1, bolVerticalSlope2, dWidth, 0);
                    pAnswerLinePoints.m_Line1.x = int0a.x;
                    pAnswerLinePoints.m_Line1.y = int0a.y;
                    //line 2 point:	line2 is below segment1 and below segment2
                    //use 3 for the orientation for Line 2
                    const int0b = LineUtility.calcTrueIntersectAbs(m1, Lowerb1, m2, Lowerb2, LinePoint2, bolVerticalSlope1, bolVerticalSlope2, dWidth, 3);
                    pAnswerLinePoints.m_Line2.x = int0b.x;
                    pAnswerLinePoints.m_Line2.y = int0b.y;
                    break;
                }
                ...
                case 1: {
                    ...
                    const int1a = LineUtility.calcTrueIntersectAbs(m1, Upperb1, m2, Lowerb2, LinePoint2, bolVerticalSlope1, bolVerticalSlope2, dWidth, 1);
                    pAnswerLinePoints.m_Line1.x = int1a.x;
                    pAnswerLinePoints.m_Line1.y = int1a.y;
                    //line2 is below segment1 and above segment2
                    //use 2 for the orientation for Line 2
                    const int1b = LineUtility.calcTrueIntersectAbs(m1, Lowerb1, m2, Upperb2, LinePoint2, bolVerticalSlope1, bolVerticalSlope2, dWidth, 2);
                    pAnswerLinePoints.m_Line2.x = int1b.x;
                    pAnswerLinePoints.m_Line2.y = int1b.y;
                    break;
                }
                ...
                case 2: {
                    ...
                    const int2a = LineUtility.calcTrueIntersectAbs(m1, Lowerb1, m2, Upperb2, LinePoint2, bolVerticalSlope1, bolVerticalSlope2, dWidth, 2);
                    pAnswerLinePoints.m_Line1.x = int2a.x;
                    pAnswerLinePoints.m_Line1.y = int2a.y;
                    //line2 is above segment1 and below segment2
                    //use 1 for the orientation for Line 1
                    const int2b = LineUtility.calcTrueIntersectAbs(m1, Upperb1, m2, Lowerb2, LinePoint2, bolVerticalSlope1, bolVerticalSlope2, dWidth, 1);
                    pAnswerLinePoints.m_Line2.x = int2b.x;
                    pAnswerLinePoints.m_Line2.y = int2b.y;
                    break;
                }
                ...
                case 3: {
                    ...
                    const int3a = LineUtility.calcTrueIntersectAbs(m1, Lowerb1, m2, Lowerb2, LinePoint2, bolVerticalSlope1, bolVerticalSlope2, dWidth, 3);
                    pAnswerLinePoints.m_Line1.x = int3a.x;
                    pAnswerLinePoints.m_Line1.y = int3a.y;
                    //line2 is above segment1 and above segment2
                    //use 0 for the orientation for Line 2
                    const int3b = LineUtility.calcTrueIntersectAbs(m1, Upperb1, m2, Upperb2, LinePoint2, bolVerticalSlope1, bolVerticalSlope2, dWidth, 0);
                    pAnswerLinePoints.m_Line2.x = int3b.x;
                    pAnswerLinePoints.m_Line2.y = int3b.y;
                    break;
                }
    ...
    //            double dist=LineUtility.calcDistance(lastPoint, nextToLastPoint);
    //            
    //            if(dist<45)
    //            {
    //                nextToLastPoint=LineUtility.extendAlongLine(lastPoint,nextToLastPoint,45+2*dist);
    ...
    //            }
    //            //lastPoint=LineUtility.extendAlongLine(lastPoint, nextToLastPoint, 45);
    //            else
    //                lastPoint=LineUtility.ExtendLineDouble(nextToLastPoint,lastPoint, -45);
    //            
    ...
    //            double dist=LineUtility.calcDistance(pLinePoints[0], pLinePoints[1]);
    //            if(dist<=45)
    //                return;
     ...
     * @return
     */
    public static GetChannel1Double(tg: TacticalGraphic,
        lpsaUpperVBPoints: number[],
        lpsaLowerVBPoints: number[],
        resultVBPoints: number[],
        vblUpperCounter: number,
        vblLowerCounter: number,
        vblChannelWidth: number,
        useptr: number,
        shapes: Array<Shape2>): number {
        let lResult: number = -1;
        try {
            let vbiDrawThis: number = tg.lineType;
            let k: number = 0;
            let vblCounter: number = 0;
            let nPrinter: number = 1;
            let
                nArrowSize: number = 40 * nPrinter;
            let
                max: number = 0;
            let dist: number = 0;
            let remainder: number = 0;
            let vblUpperCounter2: number = vblUpperCounter;
            let vblLowerCounter2: number = vblLowerCounter;
            let nReverseUpper: number = 0;
            let lUpperFlotCount: number = 0;
            let lLowerFlotCount: number = 0;
            let nLowerCounter: number = 0;
            let lUpperCounter: number = 0;
            let lResultCounter: number = 0;
            ...
            let arrowOffsetFactor: number = vblChannelWidth / 4;  //diagnostic was 10
            ...
            let lastSegmentLength: number = LineUtility.calcDistance(lastPoint, nextToLastPoint);
            ...
            switch (vbiDrawThis) {
                ...
                case TacticalLines.CATKBYFIRE: {	//80
                    ...
                    pUpperLinePoints[vblUpperCounter - 1] = LineUtility.extendAlongLine(pUpperLinePoints[vblUpperCounter - 1], pUpperLinePoints[vblUpperCounter - 2], dist);
                    pLowerLinePoints[vblLowerCounter - 1] = LineUtility.extendAlongLine(pLowerLinePoints[vblLowerCounter - 1], pLowerLinePoints[vblLowerCounter - 2], dist);
                    break;
                }
            ...
            switch (vbiDrawThis) {
                        ...
                        if (vbiDrawThis === TacticalLines.SINGLEC) {
                            ...
                            if (vbiDrawThis === TacticalLines.DOUBLEC) {
                                for (j = 0; j < pUpperLinePoints.length; j++) {
                                    pUpperLinePoints[j] = LineUtility.midPoint(pLowerLinePoints[j], pOriginalLinePoints[j], 0);
                                }
                ...
                case TacticalLines.LC: {
                    ...
                    if ((pUpperLinePoints[0].x > pUpperLinePoints[1].x) && (pUpperLinePoints[0].y !== pUpperLinePoints[1].y)) {
                        nReverseUpper = 1;
                        LineUtility.reversePoints(pLowerLinePoints, vblLowerCounter);
                    } else {
                        if ((pUpperLinePoints[0].x > pUpperLinePoints[1].x) && (pUpperLinePoints[0].y === pUpperLinePoints[1].y)) {
                            nReverseUpper = 0;
                            LineUtility.reversePoints(pUpperLinePoints, vblUpperCounter);
                        } else {
            ...
            switch (vbiDrawThis) {
                case TacticalLines.LC: {
                    lUpperFlotCount = flot.GetFlotCountDouble(pUpperLinePoints, arraysupport.getScaledSize(20, tg.lineThickness, tg.patternScale), vblUpperCounter);
                    lLowerFlotCount = flot.GetFlotCountDouble(pLowerLinePoints, arraysupport.getScaledSize(20, tg.lineThickness, tg.patternScale), vblLowerCounter);
                    ...
                    lUpperFlotCount = flot.GetFlotDouble(pUpperFlotPoints, arraysupport.getScaledSize(20, tg.lineThickness, tg.patternScale), vblUpperCounter);	//6/24/04
                    lLowerFlotCount = flot.GetFlotDouble(pLowerFlotPoints, arraysupport.getScaledSize(20, tg.lineThickness, tg.patternScale), vblLowerCounter);	//6/24/04
                ...
                case TacticalLines.CHANNEL_DASHED: {
                    ...
                    if (vbiDrawThis != TacticalLines.BBS_LINE &&
                        ...
                        for (j = 0; j < vblUpperCounter - 1; j++) {
                            let dIncrement: number = xSize + arraysupport.getScaledSize(2, tg.lineThickness / 2, tg.patternScale);
                            ...
                            for (k = 0; k < lHowManyThisSegment; k++) {
                                ...
                                if (shiftLines === true && vbiDrawThis === TacticalLines.DOUBLEC) {
                                    ptCenter.x = pUpperLinePoints[j].x + Math.trunc((f) as number * (pUpperLinePoints[j + 1].x as number - pUpperLinePoints[j].x as number) / lHowManyThisSegment as number);
                                    ptCenter.y = pUpperLinePoints[j].y + Math.trunc((f) as number * (pUpperLinePoints[j + 1].y as number - pUpperLinePoints[j].y as number) / lHowManyThisSegment as number);
                                }
                                else {
                                    if (shiftLines === false) {
                                        ptCenter.x = pOriginalLinePoints[j].x + Math.trunc((f) as number * (pOriginalLinePoints[j + 1].x as number - pOriginalLinePoints[j].x as number) / lHowManyThisSegment as number);
                                        ptCenter.y = pOriginalLinePoints[j].y + Math.trunc((f) as number * (pOriginalLinePoints[j + 1].y as number - pOriginalLinePoints[j].y as number) / lHowManyThisSegment as number);
                                    }
                                    else {
                                        ptCenter.x = pUpperLinePoints[j].x + Math.trunc((f) as number * (pUpperLinePoints[j + 1].x as number - pUpperLinePoints[j].x as number) / lHowManyThisSegment as number);
                                        ptCenter.y = pUpperLinePoints[j].y + Math.trunc((f) as number * (pUpperLinePoints[j + 1].y as number - pUpperLinePoints[j].y as number) / lHowManyThisSegment as number);
                                        let ptCenter2: POINT2 = new POINT2();
                                        ptCenter2.x = pLowerLinePoints[j].x + Math.trunc((f) as number * (pLowerLinePoints[j + 1].x as number - pLowerLinePoints[j].x as number) / lHowManyThisSegment as number);
                                        ptCenter2.y = pLowerLinePoints[j].y + Math.trunc((f) as number * (pLowerLinePoints[j + 1].y as number - pLowerLinePoints[j].y as number) / lHowManyThisSegment as number);
                                        ptCenter = LineUtility.midPoint(ptCenter, ptCenter2, 0);
                                    }
                                }
                ...
                case TacticalLines.MAIN_STRAIGHT: {
                    ...
                    if (vbiDrawThis === TacticalLines.AAAAA) {
                        ...
                        let bolVerticalUpper: number = 0;
                        let bolVerticalLower: number = 0;
                        let bUpper: number = 0;
                        let bLower: number = 0;

                        pt0 = new POINT2(pLowerLinePoints[vblLowerCounter - 2]);
                        pt1 = new POINT2(pLowerLinePoints[vblLowerCounter - 1]);
                        let dist1: number = LineUtility.calcDistance(pt0, pt1);

                        const slopeLower = LineUtility.calcTrueSlope(pt0, pt1);
                        bolVerticalLower = slopeLower.result;
                        const mLowerVal: number = slopeLower.slope;
                        bLower = pt0.y - mLowerVal * pt0.x;

                        pt0 = new POINT2(pUpperLinePoints[vblUpperCounter - 2]);
                        pt1 = new POINT2(pUpperLinePoints[vblUpperCounter - 1]);
                        const slopeUpper = LineUtility.calcTrueSlope(pt0, pt1);
                        bolVerticalUpper = slopeUpper.result;
                        const mUpperVal: number = slopeUpper.slope;
                        bUpper = pt0.y - mUpperVal * pt0.x;
                        let dist2: number = LineUtility.calcDistance(pt0, pt1);

                        //if (dist1 > vblChannelWidth && dist2 > vblChannelWidth)
                        //{
                        midPt1 = LineUtility.calcTrueIntersect(mLowerVal, bLower, mUpperVal, bUpper, bolVerticalLower, bolVerticalUpper, pt0.x, pt0.y);
                        ...
                        if (dist1 <= vblChannelWidth || dist2 <= vblChannelWidth) {
                            rotaryTooShort = true;
                            midPt1 = LineUtility.midPoint(pt0, pt1, 0);
                        }

                        a = LineUtility.calcDistance(pt0, pt1);
                        b = 30;
                        if (a < 90) {
                            b = a / 3;
                        }
                        ...
                        LineUtility.GetArrowHead4Double(pt0, pt1, d as number, d as number, arrowPts, 0);
                        ...
                        //get the base points
                        pt3 = LineUtility.extendTrueLinePerp(pt0, pt1, pt0, d / 2, 0);
                        pt4 = LineUtility.extendTrueLinePerp(pt0, pt1, pt0, -d / 2, 0);
                        ...
                        pt0 = new POINT2(pLowerLinePoints[vblLowerCounter - 2]);
                        pt1 = new POINT2(pLowerLinePoints[vblLowerCounter - 1]);
                        pt3 = LineUtility.extendLine(pt0, midPt1, b, 0);	//line distance from midpt, a was 30
                        ...
                        pt3 = LineUtility.extendLine(pt0, midPt1, b, 5);	//line distance from midpt, a was 30
                        pLinePoints[vblLowerCounter + vblUpperCounter + 16] = new POINT2(pt3);

                        //third point
                        pt0 = new POINT2(pLowerLinePoints[vblLowerCounter - 2]);
                        pt1 = new POINT2(pLowerLinePoints[vblLowerCounter - 1]);
                        pt3 = LineUtility.extendLine(pt1, midPt1, b, 0);	//line distance from midpt, a was 30
                        pLinePoints[vblLowerCounter + vblUpperCounter + 17] = new POINT2(pt3);

                        //fourth point
                        pt0 = new POINT2(pUpperLinePoints[vblLowerCounter - 2]);
                        pt1 = new POINT2(pUpperLinePoints[vblLowerCounter - 1]);
                        pt3 = LineUtility.extendLine(pt1, midPt1, b, 5);	//line distance from midpt, a was 30
                    ...
                    if (vbiDrawThis === TacticalLines.CATKBYFIRE) {	    //dist is the distance to the back of the arrowhead                        
                        ...
                        //dist-=45;
                        //end section
                        dist2 = LineUtility.calcDistance(nextToLastPoint, lastPoint);
                        if (dist2 > 45) {
                        ...
                        //was dist
                        pt3 = LineUtility.extendLine(pt2, pt1, 10 + Math.abs(dist / 2), 18); //vert height of rotary from horiz segment was dist/2.5
                        pt4 = LineUtility.extendLine(pt1, pt2, 10 + Math.abs(dist / 2), 5); //vert height of rotary from horiz segment was dist/2.5
                        midPt1 = LineUtility.midPoint(pt1, pt2, 17);
                        ...
                        pt3 = LineUtility.extendLine(pt2, pt1, Math.abs(dist / 2), 18);//vert height of rotary from horiz segment was dist/2.5
                        pt4 = LineUtility.extendLine(pt1, pt2, Math.abs(dist / 2), 18);//vert height of rotary from horiz segment was dist/2.5

                        midPt2 = LineUtility.midPoint(pt1, pt2, 18);
                        ...
                        if (midPt1.x === midPt2.x && midPt1.y === midPt2.y) //last segment too short
                        {
                            ...
                            midPt1 = LineUtility.extendLine(nextToLastPoint, pArrowLinePoints[0], 10, 17);
                            //pt1 = LineUtility.extendTrueLinePerp(lastPoint, midPt1, midPt1, 30, 18);
                            //pt2 = LineUtility.extendTrueLinePerp(lastPoint, midPt1, midPt1, -30, 5);                            
                            pt1 = LineUtility.extendTrueLinePerp(lastPoint, midPt1, midPt1, dFeature, 18);
                            pt2 = LineUtility.extendTrueLinePerp(lastPoint, midPt1, midPt1, -dFeature, 5);
                            //end section
                            ...
                            if (Channels._client.startsWith("cpof")) {
                                ...
                                midPt2 = LineUtility.extendLine(nextToLastPoint, pArrowLinePoints[0], 20, 17);
                            }
                                ...
                                if (dist2 > 30) {
                                    ...
                                    midPt2 = LineUtility.extendLine(nextToLastPoint, pArrowLinePoints[0], dFeature, 17);
                                }
                            ...
                            //diagnostic 2-27-13                            
                            //pt1 = LineUtility.extendTrueLinePerp(lastPoint, midPt2, midPt2, 20, 18);
                            //pt2 = LineUtility.extendTrueLinePerp(lastPoint, midPt2, midPt2, -20, 18);
                            dFeature -= 10;
                            pt1 = LineUtility.extendTrueLinePerp(lastPoint, midPt2, midPt2, dFeature, 18);
                            pt2 = LineUtility.extendTrueLinePerp(lastPoint, midPt2, midPt2, -dFeature, 18);
                        ...
                        pt1 = LineUtility.extendLine(midPt1, midPt2, dFeature, dFeature as number); //30, then 5
                    ...
                    if (vbiDrawThis == TacticalLines.FRONTAL_ATTACK) {
                        ...
                        midPt1 = LineUtility.midPoint(pt0, pt1, 0);
                        pt0 = LineUtility.extendAlongLine(pt1, midPt1, vblChannelWidth);
                        pt1 = LineUtility.extendAlongLine(pt0, midPt1, vblChannelWidth);
                    ...
                    if (vbiDrawThis == TacticalLines.FRONTAL_ATTACK || vbiDrawThis == TacticalLines.TURNING_MOVEMENT) {
                        ...
                        pt1 = LineUtility.midPoint(pLinePoints[vblLowerCounter + vblUpperCounter + 1], pLinePoints[vblLowerCounter + vblUpperCounter + 6], 0); // top right
                        pt2 = new POINT2(pLinePoints[vblLowerCounter + vblUpperCounter+4]); // bottom left
                        pt3 = LineUtility.midPoint(pLinePoints[vblLowerCounter + vblUpperCounter + 5], pLinePoints[vblLowerCounter + vblUpperCounter + 6], 0); // bottom right

                        // Shrink vertically to add spacing between arrow lines
                        dist = LineUtility.calcDistance(pt0, pt2) / 4;
                        pt0 = LineUtility.extendAlongLine2(pt0, pt2, dist);
                        pt1 = LineUtility.extendAlongLine2(pt1, pt3, dist);
                        pt2 = LineUtility.extendAlongLine2(pt2, pt0, dist);
                        pt3 = LineUtility.extendAlongLine2(pt3, pt1, dist);

                        // Make height twice size of width
                        if (LineUtility.calcDistance(pt0, pt2) > 2 * LineUtility.calcDistance(pt0, pt1)) {
                            // Shrink vertical
                            dist = LineUtility.calcDistance(pt0, pt1);

                            midPt1 = LineUtility.midPoint(pt0, pt2, 0);
                            pt0 = LineUtility.extendAlongLine2(midPt1, pt0, dist);
                            pt2 = LineUtility.extendAlongLine2(midPt1, pt2, dist);

                            midPt1 = LineUtility.midPoint(pt1, pt3, 0);
                            pt1 = LineUtility.extendAlongLine2(midPt1, pt1, dist);
                            pt3 = LineUtility.extendAlongLine2(midPt1, pt3, dist);
                        } else if (2 * LineUtility.calcDistance(pt0, pt1) > LineUtility.calcDistance(pt0, pt2)) {
                            //Shrink horizontal
                            dist = LineUtility.calcDistance(pt0, pt2) / 2;
                            pt1 = LineUtility.extendAlongLine2(pt0, pt1, dist);
                            pt3 = LineUtility.extendAlongLine2(pt2, pt3, dist);
                        }
                        ...
                        if (vbiDrawThis == TacticalLines.FRONTAL_ATTACK) {
                            ...
                            pLinePoints[vblLowerCounter + vblUpperCounter + 11] = LineUtility.midPoint(pt0, pt1, 0);
                            pLinePoints[vblLowerCounter + vblUpperCounter + 11].style = 0;
                            pLinePoints[vblLowerCounter + vblUpperCounter + 12] = new POINT2(pt3);
                            pLinePoints[vblLowerCounter + vblUpperCounter + 12].style = 5;
                            pLinePoints[vblLowerCounter + vblUpperCounter + 13] = LineUtility.midPoint(pLinePoints[vblLowerCounter + vblUpperCounter + 10], pLinePoints[vblLowerCounter + vblUpperCounter + 11], 0);
                            pLinePoints[vblLowerCounter + vblUpperCounter + 13].style = 0;
                            pLinePoints[vblLowerCounter + vblUpperCounter + 14] = LineUtility.midPoint(pLinePoints[vblLowerCounter + vblUpperCounter + 11], pLinePoints[vblLowerCounter + vblUpperCounter + 12], 5);
                            pLinePoints[vblLowerCounter + vblUpperCounter + 14].style = 5;
                        } else {
                            // Draw "T"
                            pLinePoints[vblLowerCounter + vblUpperCounter + 10] = LineUtility.midPoint(pt0, pt1, 0);
                            pLinePoints[vblLowerCounter + vblUpperCounter + 10].style = 0;
                            pLinePoints[vblLowerCounter + vblUpperCounter + 11] = LineUtility.midPoint(pt2, pt3, 5);
            ...
            if (shapes == null) {
                ...
                for (j = 0; j < pLinePoints.length; j++) {
                    ...
                    resultVBPoints[3 * j + 2] = pLinePoints[j].style as number;
                }
     ...
     * @return
     */
    private static getAXADFillShapes(lineType: number, pLinePoints: POINT2[]): Array<Shape2> {
