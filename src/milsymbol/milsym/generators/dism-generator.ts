...
export class DISMSupport {
    private static readonly LEFT_SIDE: number = 0;
    private static readonly RIGHT_SIDE: number = 1;
    private static readonly COLINEAR: number = 2;

    private static readonly CONST_PI: number = Math.PI;
    ...
    //        minLength=mLength;
    //    }
    private static GetTGFontSize(iLength: number): number {
        let result: number = -1;
        try {
            if (iLength < 20) {
    ...
    private static ArcApproximationDouble(left: number, top: number, right: number, bottom: number,
        startx: number, starty: number, endx: number, endy: number, lpoints: POINT2[]): void {

        try {
            let dstartx: number = startx;
            let dstarty: number = starty;
            let dendx: number = endx;
            let dendy: number = endy;
            let a: number = 0;
            let b: number = 0;
            let ctrX: number = 0;
            let ctrY: number = 0;
    ...
    private static DetermineDirectionDouble(points: POINT2[]): number {
        let result: number = 0;
        try {
            let dP0P1M: number = 0;
            let iP0P1B: number = 0;
            if (points[0].x === points[1].x) {
                ...
                dP0P1M = (points[0].y - points[1].y) as number / (points[0].x - points[1].x) as number;
    ...
    private static CalcEndpieceDeltasDouble(points: POINT2[],
        dAngleDelta: number
    ): { deltaX: number, deltaY: number } {
        let deltaXVal: number = 0;
        let deltaYVal: number = 0;
        try {
            // find midpoint between point0 and point1
            let pntMid: POINT2 = new POINT2();
            let iDiagEOL_length: number = 0;
            ...
            if (iDiagEOL_length as number > DISMSupport.maxLength / 5 * DPIScaleFactor) {
            ...
            deltaXVal = (iDiagEOL_length * Math.cos(dAngle1));
            deltaYVal = (iDiagEOL_length * Math.sin(dAngle1));
        } catch (exc) {
            if (exc instanceof Error) {
     ...
     */
    static GetDelayGraphicEtcDouble(points: POINT2[], lineType: number): number {
        let counter: number = 0;
        try {
            let savepoints: POINT2[] = new Array<POINT2>(3);
            let iLength: number = 0;
            let iRadius: number = 0;
            let iDiagEOL_length: number = 0;
            let dAngle1: number = 0;
            let iDeltaX1: number = 0;
            let iDeltaY1: number = 0;
            let iDeltaX2: number = 0;
            let iDeltaY2: number = 0;
            ...
            if (lineType == TacticalLines.PURSUIT) {
                ...
                points[counter] = LineUtility.extendAlongLine(savepoints[1], savepoints[2], iRadius * 2 - iDiagEOL_length, 0);
                counter++;
                points[counter] = LineUtility.extendAlongLine(savepoints[1], savepoints[2], iRadius * 2 + iDiagEOL_length, 0);
                counter++;
            }
    ...
    private static IsEnvelopmentArcReversed(pPoints: POINT2[]): boolean {
        try {
            ...
            let d: number = LineUtility.calcDistance(ptsSeize[0], pPoints[3]);

            ptsSeize[0] = new POINT2(pPoints[2]);
            ptsSeize[1] = new POINT2(pPoints[1]);
            LineUtility.calcClockwiseCenter(ptsSeize);
            let dArcReversed: number = LineUtility.calcDistance(ptsSeize[0], pPoints[3]);
     ...
     * @param points OUT - the client points, also used for the returned points.
     */
    static GetEnvelopmentGraphicDouble(points: POINT2[]): number {
        let counter: number = 0;
        try {
            let iLength: number = 0;
            let iRadius: number = 0;
            ...
            points[2] = LineUtility.ClosestPointOnLine(points[0], LineUtility.extendLine(points[0], points[1], LineUtility.calcDistance(points[1], points[2]), 0), points[2]);
    ...
    static GetDISMCoverDouble(points: POINT2[],
        ...
        try {
            ...
            let quadrant: number = LineUtility.GetQuadrantDouble(pt0, pt4);
            ...
            let length1: number = LineUtility.calcDistance(initialPt, endPt0);
            let length2: number = LineUtility.calcDistance(initialPt, endPt1);
            length1 = Math.min(length1, length2);

            if (DISMSupport.GetTGFontSize(length1) > 0) {
                let delta: number = length1 / 15;
                ...
                // Draw jaggy line 1
                let angle0: number = Math.atan2(initialPt.y - endPt0.y, initialPt.x - endPt0.x);
                let deltaX0a: number = Math.cos(angle0 + sign * DISMSupport.CONST_PI / 4) * delta;
                let deltaY0a: number = Math.sin(angle0 + sign * DISMSupport.CONST_PI / 4) * delta;
                ...
                let midPt0: POINT2 = LineUtility.midPoint(ptsJaggyLine[0], endPt0, 0);
                ...
                // Draw Arrow 1
                let deltaX0b: number = Math.cos(angle0 - sign * DISMSupport.CONST_PI / 4) * delta;
                let deltaY0b: number = Math.sin(angle0 - sign * DISMSupport.CONST_PI / 4) * delta;
                ...
                // Draw jaggy line 2
                let angle1: number = Math.atan2(initialPt.y - endPt1.y, initialPt.x - endPt1.x);
                let deltaX1a: number = Math.cos(angle1 - sign * DISMSupport.CONST_PI / 4) * delta;
                let deltaY1a: number = Math.sin(angle1 - sign * DISMSupport.CONST_PI / 4) * delta;
                ptsJaggyLine[0] = new POINT2(initialPt);
                if (lineType !== TacticalLines.SARA) {
                    ptsJaggyLine[0].x -= 30 * Math.cos(angle1);  //was 20
                    ptsJaggyLine[0].y -= 30 * Math.sin(angle1);
                }
                let midPt1: POINT2 = LineUtility.midPoint(ptsJaggyLine[0], endPt1, 0);
                ...
                // Draw Arrow 2
                let deltaX1b: number = Math.cos(angle1 + sign * DISMSupport.CONST_PI / 4) * delta;
                let deltaY1b: number = Math.sin(angle1 + sign * DISMSupport.CONST_PI / 4) * delta;
    ...
    static GetDISMCoverDoubleRevC(points: POINT2[],
        ...
        try {
            ...
            let dDeltaX0: number = 0;
            let dDeltaY0: number = 0;
            let dDeltaX1: number = 0;
            let dDeltaY1: number = 0;
            let iLengthPt0Pt1: number = 0;
            let iLengthPt0Pt2: number = 0;
            let iDelta: number = 0;
            let j: number = 0;
            let t: number = 1;
            let iFontSize: number = 0;
            let iLetterOffset: number = 0;
            let savepoints: POINT2[] = new Array<POINT2>(3);
    ...
    static GetDISMBypassDouble(points: POINT2[],
        ...
        try {
            ...
            const { deltaX: iDeltaX, deltaY: iDeltaY } = DISMSupport.CalcEndpieceDeltasDouble(savepoints, DISMSupport.CONST_PI / 4);

            if ((savepoints[0].y - savepoints[1].y) < 0) {// Point0 is higher than Point1
                if (bPointsRight !== 0) {// figure opens to the right
                    DISMSupport.DrawEndpieceDeltasDouble(savepoints[0],
                        iDeltaX, iDeltaY, iDeltaY, -iDeltaX, deltapoints1);
                    DISMSupport.DrawEndpieceDeltasDouble(savepoints[1],
                        iDeltaX, iDeltaY, iDeltaY, -iDeltaX, deltapoints2);
                } else {// figure opens to the left
                    DISMSupport.DrawEndpieceDeltasDouble(savepoints[0],
                        iDeltaY, -iDeltaX, iDeltaX, iDeltaY, deltapoints1);
                    DISMSupport.DrawEndpieceDeltasDouble(savepoints[1],
                        iDeltaY, -iDeltaX, iDeltaX, iDeltaY, deltapoints2);
                }
     ...
     * @param linetype the line type.
     */
    static GetDISMBreachDouble(points: POINT2[], linetype: number): number {
        let counter: number = 0;
        try {
            let pointsCorner: POINT2[] = new Array<POINT2>(2);
            let rectpts: POINT2[] = new Array<POINT2>(4);
            ...
            if ((savepoints[0].y - savepoints[1].y) < 0) {// Point0 is higher than Point1
                if (bPointsRight !== 0) {// figure opens to the right
                    DISMSupport.DrawEndpieceDeltasDouble(savepoints[0],
                        iDeltaX, iDeltaY, -iDeltaX, -iDeltaY, deltapoints1);
                    DISMSupport.DrawEndpieceDeltasDouble(savepoints[1],
                        iDeltaY, -iDeltaX, -iDeltaY, iDeltaX, deltapoints2);
                } else {// figure opens to the left
                    DISMSupport.DrawEndpieceDeltasDouble(savepoints[0],
                        iDeltaY, -iDeltaX, -iDeltaY, iDeltaX, deltapoints1);
                    DISMSupport.DrawEndpieceDeltasDouble(savepoints[1],
                        iDeltaX, iDeltaY, -iDeltaX, -iDeltaY, deltapoints2);
                }
            } else {// Point0 is lower than Point1
                if (bPointsRight !== 0) {// figure opens to the right
                    DISMSupport.DrawEndpieceDeltasDouble(savepoints[0],
     ...
     * @param linetype the line type.
     */
    static GetDISMCanalizeDouble(points: POINT2[], linetype: number): number {
        let counter: number = 0;
        try {
            let pointsCorner: POINT2[] = new Array<POINT2>(2);
            let rectpts: POINT2[] = new Array<POINT2>(4);
     ...
     */
    public static getFDIShape(tg: TacticalGraphic, ptA: POINT2, ptC: POINT2): Shape2;
     ...
     * @return Dummy indicator shape
     */
    public static getFDIShape(tg: TacticalGraphic, ptA: POINT2, ptB: POINT2, ptC: POINT2): Shape2;
    public static getFDIShape(...args: unknown[]): Shape2 | null {
        switch (args.length) {
            case 3: {
                const [tg, ptA, ptC] = args as [TacticalGraphic, POINT2, POINT2];


                try {
                    let midPt: POINT2 = LineUtility.midPoint(ptA, ptC, 0);
                    let len: number = LineUtility.calcDistance(ptA, midPt);
                    ...
                    let stroke: BasicStroke = TacticalUtils.getLineStroke(tg.lineThickness, shape.style, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_MITER);
            ...
            case 4: {
                let [tg, ptA, ptB, ptC] = args as [TacticalGraphic, POINT2, POINT2, POINT2];


                try {
                    // Extend ptA and ptC .25w
                    let w: number = LineUtility.calcDistance(ptA, ptC) * 0.25;
                    if (w < tg.lineThickness * 1.5) {
                        // lineThickness * 1.5 is minimum distance between arrow and dummy modifier
                        w = tg.lineThickness * 1.5;
                    }
                    ...
                    // Extend ptB .5w
                    let midPt: POINT2 = LineUtility.midPoint(ptA, ptC, 0);
                    w = LineUtility.calcDistance(midPt, ptB) * 0.5;
                    if (w < tg.lineThickness * 1.5) {
                        // lineThickness * 1.5 is minimum distance between arrow and dummy modifier
                        w = tg.lineThickness * 1.5;
                    }
     ...
     * @param linetype the line type.
     */
    static GetDISMDisruptDouble(points: POINT2[], linetype: number): number {
        let counter: number = 0;
        try {
            let pts: POINT2[] = new Array<POINT2>(2);
     ...
     * @param linetype the line type.
     */
    static GetDISMContainDouble(points: POINT2[], linetype: number): number {
        let counter: number = 0;
        try {
            let pts: POINT2[] = new Array<POINT2>(3);
            let ptCenter: POINT2 = new POINT2();
            let ptPerp: POINT2 = new POINT2(); // point used to draw perpendicular line
            let iPerpLength: number = 0;
            let j: number = 0;
            let dAngle1: number = 0;
            let d: number = 0;
            let dCosAngle1: number = 0;
            let dSinAngle1: number = 0;
            let iRadius: number = 0;
            let iDiagEOL_length: number = 0;
            let dAngle2: number = 0;
            let dDeltaX1: number = 0;
            let dDeltaY1: number = 0;
            let dDeltaX2: number = 0;
            let dDeltaY2: number = 0;
            let savepoints: POINT2[] = new Array<POINT2>(3);
            let arcpoints: POINT2[] = new Array<POINT2>(17);
            ...
            const { slope: mVal } = LineUtility.calcTrueSlope2(savepoints[0], savepoints[1]);
            if (mVal !== 0) {
            ...
            d = LineUtility.calcDistance(pts[0], pts[1]);
     ...
     * @param linetype the line type.
     */
    static GetDISMFixDouble(points: POINT2[], linetype: number, clipBounds: Rectangle2D | null): number {
        let counter: number = 0;
        try {
            let pts: POINT2[] = new Array<POINT2>(3);
            ...
            let dJaggyHalfAmp: number = 0;
            let dJaggyHalfPeriod: number = 0;
            let dDeltaXOut: number = 0;
            let dDeltaYOut: number = 0;
            let dDeltaXAlong: number = 0;
            let dDeltaYAlong: number = 0;
            let iNumJaggies: number = 0;
            let i: number = 0;
            let j: number = 0;
     ...
     * @param linetype the line type.
     */
    static GetDISMClearDouble(points: POINT2[], linetype: number): number {
        let counter: number = 0;
        try {
            let savepoints: POINT2[] = new Array<POINT2>(3);
            let j: number = 0;
            let pts: POINT2[] = new Array<POINT2>(2);
            let ptsArrow: POINT2[] = new Array<POINT2>(3);
            let ctrX: number = ((points[0].x + points[1].x) / 2);
            let ctrY: number = ((points[0].y + points[1].y) / 2);
            let deltapoints1: POINT2[] = new Array<POINT2>(4);
            let deltapoints2: POINT2[] = new Array<POINT2>(4);
            let deltapoints3: POINT2[] = new Array<POINT2>(4);
            ...
            const { deltaX: iDeltaX1, deltaY: iDeltaY1 } = DISMSupport.CalcEndpieceDeltasDouble(savepoints, DISMSupport.CONST_PI / 6);
            const { deltaX: iDeltaX2, deltaY: iDeltaY2 } = DISMSupport.CalcEndpieceDeltasDouble(savepoints, -DISMSupport.CONST_PI / 6);
            DISMSupport.DrawEndpieceDeltasDouble(ptsArrow[0],
                iDeltaX1, iDeltaY1, iDeltaX2, iDeltaY2, deltapoints1);
            DISMSupport.DrawEndpieceDeltasDouble(ptsArrow[1],
                iDeltaX1, iDeltaY1, iDeltaX2, iDeltaY2, deltapoints2);
    ...
    private static IsSeizeArcReversed(pPoints: POINT2[]): boolean {
        try {
            let dAngle1: number = Math.atan2(pPoints[0].y - pPoints[1].y, pPoints[0].x - pPoints[1].x);
            let dDeltaX1: number = Math.cos(dAngle1 + DISMSupport.CONST_PI / 4);
            let dDeltaY1: number = Math.sin(dAngle1 + DISMSupport.CONST_PI / 4);
            let dDeltaX2: number = Math.cos(dAngle1 - DISMSupport.CONST_PI / 4);
            let dDeltaY2: number = Math.sin(dAngle1 - DISMSupport.CONST_PI / 4);

            let dChordLength: number = Math.sqrt((pPoints[1].x - pPoints[0].x) * (pPoints[1].x - pPoints[0].x) +
                (pPoints[1].y - pPoints[0].y) * (pPoints[1].y - pPoints[0].y));
            let dArcRadius: number = dChordLength / 1.414213562373; // sqrt(2) == 1.414213562373
            let ptArcCenter: POINT2 = new POINT2();

            //get the default center
            ptArcCenter.x = pPoints[0].x - dDeltaX1 * dArcRadius;
            ptArcCenter.y = pPoints[0].y - dDeltaY1 * dArcRadius;
            let d: number = LineUtility.calcDistance(ptArcCenter, pPoints[2]);
            ...
            let dReversed: number = LineUtility.calcDistance(ptArcCenterReversed, pPoints[2]);
    ...
    static GetDISMSeizeDouble(points: POINT2[],
        ...
        try {
            ...
            let iCircleRadius: number = (25 * scale);
            let arcpoints: POINT2[] = new Array<POINT2>(17);
            let pts: POINT2[] = new Array<POINT2>(3);
            let dAngle1: number = 0;
            let dDeltaX1: number = 0;
            ...
            let dDeltaX2: number = 0;
            let dDeltaY2: number = 0;
            let dChordLength: number = 0;
            let dArcRadius: number = 0;
            let j: number = 0;
            let dDeltaX3: number = 0;
            let dDeltaY3: number = 0;
            let iDiagEOL_length: number = 0;
            let factor: number = 1;
            ...
            if (!client.startsWith("cpof") && radius === 0) {
                dArcRadius = LineUtility.calcDistance(savepoints[0], savepoints[1]);
            ...
            if (isArcReversed === false) {
                ...
                for (j = 0; j < 17; j++) {
                    if (LineUtility.calcDistance(savepoints[0], arcpoints[j]) >= iCircleRadius) {
                        points[counter] = new POINT2(arcpoints[j]);
                        points[counter].style = 0;
                        counter++;
                    } else if (j > 0) {
            ...
            } else //arc is reversed
            {
                ...
                for (j = 0; j < 17; j++) {
                    ...
                    if (outsideCircle || LineUtility.calcDistance(savepoints[0], arcpoints[j]) >= iCircleRadius) {
     ...
     */
    private static side(x1: number, y1: number, x2: number, y2: number, px: number, py: number): number {
        let dx1: number = 0;
        let dx2: number = 0;
        let dy1: number = 0;
        let dy2: number = 0;
     ...
     * @param linetype the line type
     */
    static GetDISMRIPDouble(points: POINT2[], linetype: number): number {
        let counter: number = 0;
        try {
            ...
            let side01: number = DISMSupport.side(points[0].x, points[0].y, points[1].x, points[1].y, points[2].x, points[2].y);
            let side12: number = DISMSupport.side(points[1].x, points[1].y, points[2].x, points[2].y, points[3].x, points[3].y);
    ...
    static GetDISMByDifDouble(points: POINT2[],
        linetype: number,
        clipBounds: Rectangle2D | null): number {
        let counter: number = 0;
        try {
            let pointsCorner: POINT2[] = new Array<POINT2>(2);
            let rectpts: POINT2[] = new Array<POINT2>(4);
     ...
     * @param linetype the line type.
     */
    static GetDISMPenetrateDouble(points: POINT2[], linetype: number): void {
        try {
            ...
            midpt = LineUtility.midPoint(savepoints[0], savepoints[1], 0);
    ...
    static GetDISMByImpDouble(points: POINT2[],
        ...
        try {
            ...
            let bPointsRight: number = 0;
            let dMBR: number = LineUtility.MBRDistance(points, 3);
            //end declarations
            ...
            if (dMBR / 15 > LineUtility.calcDistance(points[0], points[1])) {

                // Don't let gap be wider than channel
                dMBR = 15 * LineUtility.calcDistance(points[0], points[1]);
            }
            ...
            points[counter].style = 0;
            counter++;
            midpt = LineUtility.midPoint(rectpts[1], rectpts[2], 0);
            pts[0] = LineUtility.extendLine(rectpts[1], midpt, -dMBR / 30, 5);
            pts[1] = LineUtility.extendLine(rectpts[1], midpt, dMBR / 30, 5);
            points[counter] = new POINT2(pts[0]);
            points[counter].style = 5;
            counter++;
    ...
    private static ReorderAtkByFirePoints(points: POINT2[]): void {
        try {
            ...
            let distToLine: number = 0;
            let distanceToPointAboveLine: number = 0;
            let distanceToPointBelowLine: number = 0;
            let distanceToPointLeftOfLine: number = 0;
            let distanceToPointRightOfLine: number = 0;
            for (let j: number = 0; j < 3; j++) {
                savepoints[j] = new POINT2(points[j]);
            }

            if (Math.abs(savepoints[1].x - savepoints[2].x) > 2) {
                distToLine = LineUtility.calcDistanceToLine(savepoints[1], savepoints[2], savepoints[0]);
                ...
                distanceToPointAboveLine = LineUtility.calcDistance(savepoints[0], ptAboveLine);
                distanceToPointBelowLine = LineUtility.calcDistance(savepoints[0], ptBelowLine);
            ...
            } else //the last 2 points form a vertical line
            {
                ...
                distanceToPointLeftOfLine = LineUtility.calcDistance(savepoints[0], ptLeftOfLine);
                distanceToPointRightOfLine = LineUtility.calcDistance(savepoints[0], ptRightOfLine);
    ...
    private static ReorderSptByFirePoints(points: POINT2[]): void {
        try {
            ...
            let midpt: POINT2 = LineUtility.midPoint(points[0], points[1], 0);
            if (Math.abs(points[2].x - points[3].x) > 2) {
                distToLine = LineUtility.calcDistanceToLine(points[1], points[2], midpt);
                ...
                distanceToPointAboveLine = LineUtility.calcDistance(points[0], ptAboveLine);
                distanceToPointBelowLine = LineUtility.calcDistance(points[0], ptBelowLine);
                if (distanceToPointAboveLine < distanceToPointBelowLine) {
                    //then pt2 - pt3 should be left to right
            ...
            } else //the last 2 points form a vertical line
            {
                ...
                distanceToPointLeftOfLine = LineUtility.calcDistance(points[0], ptLeftOfLine);
                distanceToPointRightOfLine = LineUtility.calcDistance(points[0], ptRightOfLine);
     ...
     * @param linetype the line type.
     */
    static GetDISMATKBYFIREDouble(points: POINT2[], linetype: number): number {
        let counter: number = 0;
        try {
            let pts: POINT2[] = new Array<POINT2>(3);
     ...
     * @param linetype the line type.
     */
    static GetDISMGapDouble(points: POINT2[], linetype: number): number {
        try {
            let savepoints: POINT2[] = new Array<POINT2>(4);
            let pts: POINT2[] = new Array<POINT2>(2);
            let j: number = 0;
            let dMBR: number = LineUtility.MBRDistance(points, 4);
            //end declarations
            ...
            points[4].style = 0;
            points[5] = LineUtility.midPoint(pts[0], pts[1], 5);
            //get the extension point
            ...
            points[6].style = 0;
            points[7] = LineUtility.midPoint(pts[0], pts[1], 5);
            //get the extension point
            ...
            points[8].style = 0;
            points[9] = LineUtility.midPoint(pts[0], pts[1], 5);
            //get the extension point
            ...
            points[10].style = 0;
            points[11] = LineUtility.midPoint(pts[0], pts[1], 5);
     ...
     * @param linetype the line type.
     */
    static GetDISMMinefieldDisruptDouble(points: POINT2[], linetype: number): number {
        let counter: number = 0;
        try {
            let pts: POINT2[] = new Array<POINT2>(2);
            let ptsArrow: POINT2[] = new Array<POINT2>(3);
            ...
            //draw tail
            dist = LineUtility.calcDistance(savepoints[2], savepoints[0]);
            d = dist;
            let DPIScaleFactor: number = rendererSettings.getDeviceDPI() / 96.0;
            if (d > 5 * DISMSupport.maxLength * DPIScaleFactor) {
                d = 5 * DISMSupport.maxLength * DPIScaleFactor;
     ...
     */
    static GetDISMLinearTargetDouble(points: POINT2[], linetype: number, vblCounter: number): number {
        let counter: number = 0;
        try {
            let j: number = 0;
            let dMBR: number = LineUtility.MBRDistance(points, vblCounter - 4);
            //end declarations
            ...
            points[counter] = LineUtility.extendTrueLinePerp(points[0], points[1], points[0], dMBR / 20, 0);
            counter++;
            points[counter] = LineUtility.extendTrueLinePerp(points[0], points[1], points[0], -dMBR / 20, 5);
            counter++;
            points[counter] = LineUtility.extendTrueLinePerp(points[vblCounter - 5], points[vblCounter - 6], points[vblCounter - 5], dMBR / 20, 0);
            counter++;
            points[counter] = LineUtility.extendTrueLinePerp(points[vblCounter - 5], points[vblCounter - 6], points[vblCounter - 5], -dMBR / 20, 5);
            counter++;
     ...
     * @param linetype the line type.
     */
    protected static GetDISMPAADouble(points: POINT2[], linetype: number): void {
        try {
            let pt0: POINT2 = new POINT2(points[0]);
            let pt1: POINT2 = new POINT2(points[1]);
            ...
            let pt3: POINT2 = new POINT2();
            let midpt: POINT2 = new POINT2();
            let d: number = LineUtility.calcDistance(pt0, pt1);

            midpt = LineUtility.midPoint(pt0, pt1, 0);
            pt2 = LineUtility.extendTrueLinePerp(pt0, pt1, midpt, d / 2, 0);
            pt3 = LineUtility.extendTrueLinePerp(pt0, pt1, midpt, -d / 2, 0);
            d = LineUtility.calcDistance(pt0, pt2);
    ...
    private static ReverseDelayArc(points: POINT2[]): boolean {
        ...
        let lineAngle: number = DISMSupport.getAngleBetweenPoints(pt1.x, pt1.y, pt2.x, pt2.y);
        let curveAngle: number = DISMSupport.getAngleBetweenPoints(pt2.x, pt2.y, pt3.x, pt3.y);

        let upperBound: number = curveAngle + 180;
        return !DISMSupport.isInRange(curveAngle, upperBound, lineAngle);
    }

    private static isInRange(min: number, max: number, targetAngle: number): boolean {
    ...
    private static getAngleBetweenPoints(x1: number, y1: number, x2: number, y2: number): number {
     ...
     */
    static AmbushPointsDouble(pLinePoints: POINT2[]): number {
        let counter: number = 0;
        try {
            let pts: POINT2[] = new Array<POINT2>(3);
            let savepoints: POINT2[] = new Array<POINT2>(3);
            ...
            let d: number = 0;
            let dAngle1: number = 0;
            let dAngle1c: number = 0;
            let dAngle2c: number = 0;
            let dAngle12c: number = 0;
            let dAngle0: number = 0;
            let arcpoints: POINT2[] = new Array<POINT2>(17);
            let dAngleTic: number = 0;
            let dDeltaX1: number = 0;
            let dDeltaY1: number = 0;
            let dDeltaX2: number = 0;
            let dDeltaY2: number = 0;
            ...
            let dRadius2: number = Math.sqrt((ptMid.x - savepoints[1].x) * (ptMid.x - savepoints[1].x) +
            ...
            let dAngle2: number = Math.atan2(savepoints[2].y - savepoints[1].y, savepoints[2].x - savepoints[1].x);
            ...
            let dist: number = LineUtility.calcDistance(savepoints[0], ptCenter);
            let dist2: number = LineUtility.calcDistance(savepoints[0], ptCenter2);
            ...
            if (iArrowLength > DISMSupport.maxLength * DPIScaleFactor) {
                ...
                iArrowLength = DISMSupport.maxLength as number * DPIScaleFactor;
            }

            if (iArrowLength < DISMSupport.minLength * DPIScaleFactor) {

                iArrowLength = DISMSupport.minLength as number * DPIScaleFactor;
            }
