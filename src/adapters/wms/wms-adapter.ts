...
function negotiateCrs(supportedCrs: string[]): string {
  const upper = new Set(supportedCrs.map((c) => c.toUpperCase()));
  if (upper.has('EPSG:3857')) return 'EPSG:3857';
  if (upper.has('EPSG:900913')) return 'EPSG:900913';
  if (upper.has('EPSG:4326')) return 'EPSG:4326';
