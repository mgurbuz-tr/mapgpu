...
function parseBoundingBoxes(layerEl: Element): WmsBoundingBox[] {
  return directChildren(layerEl, 'BoundingBox').map((bbEl) => {
    ...
    const minx = Number.parseFloat(bbEl.getAttribute('minx') ?? '0');
    const miny = Number.parseFloat(bbEl.getAttribute('miny') ?? '0');
    const maxx = Number.parseFloat(bbEl.getAttribute('maxx') ?? '0');
    const maxy = Number.parseFloat(bbEl.getAttribute('maxy') ?? '0');
...
function parseTimeDimension( // NOSONAR
  layerEl: Element,
  is130: boolean,
