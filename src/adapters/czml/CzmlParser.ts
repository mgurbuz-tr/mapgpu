...
function _parseEntityPacket(p: Record<string, unknown>): CzmlFeature | null { // NOSONAR
    ...
    if (position['cartographicDegrees'] && Array.isArray(position['cartographicDegrees'])) {
      ...
      if (cd.length === 3) {
        ...
        if (position['epoch']) {
          ...
          geometry = { type: 'Point', coordinates: [cd[0]!, cd[1]!, cd[2]!] }; // NOSONAR — bounds checked via cd.length
        }
      } else if (cd.length > 4) {
      ...
      if (cart.length >= 3) {
        ...
        const alt = Math.hypot(x, y, z) - 6378137;
